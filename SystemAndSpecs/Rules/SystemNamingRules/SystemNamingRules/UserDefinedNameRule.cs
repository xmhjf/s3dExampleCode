...
namespace Ingr.SP3D.Content.System.Rules
{
    ...
    public class UserDefinedNameRule : NameRuleBase 
    {
        ...
        //These error codes have been added in order to send proper error messages to be handled by client command and notify user accordingly

        const int errCode = -2147221504;
        const int existingName = 4 + errCode + 512;
        const int emptyName = 4 + errCode + 513;
        
        

        public UserDefinedNameRule()
        {
            InitStrings();
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="entity">Input.  System object whose name is being checked.</param>
        /// <param name="parents">Input.  Parent system of oEntity.</param>
        /// <param name="activeEntity">Input.  Naming active entity.</param>
        public override void ComputeName(BusinessObject entity, ReadOnlyCollection<BusinessObject> parents, BusinessObject activeEntity)
        {
            ...
            try
            {
                ...
                if (IsNameBlank(entity, ref newName))
                {
                    ...
                    MiddleServiceProvider.ErrorLogger.Log(emptyName, "ComputeName", blankName + newName, "NAMING", null, null, null, 0, entity, 0);
                }
                else
                {
                    string origName = string.Empty;
                    if (TestForDuplicateName(entity, ref origName, ref newName))
                    {
                        ...
                        MiddleServiceProvider.ErrorLogger.Log(existingName, "ComputeName", duplicateName + origName + hasChanged + newName, "NAMING", null, null, null, 0, entity, 0);
            ...
            {
                throw new Exception("Unexpected error:  SystemNamingRule.UserDefinedNameRule.ComputeName");
            }
        }
            ...
            try
            {
                ...
                if (oChildren != null)
                {
                    for (iNdx = 0; iNdx < oChildren.Count; iNdx++)
                    {
                        ...
                        if (null != oChild)
                            strChildName = base.GetName(oChild);

                        // Be sure not to compare the object with itself.
                        // TR-CP-283172  Investigate record exception of DeleteSketchedContourATP in 11.00.82.0102: Check for null string to avoid Access Violation exception.
                        long lCurrSubscript = 0;
                        if (oChild != null)
                        {
                            if (!string.IsNullOrEmpty(strChildName) && !oChild.Equals(system))
                            {

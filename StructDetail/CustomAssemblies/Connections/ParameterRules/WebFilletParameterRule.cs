...
//
//File
//  WebFilletParameterRule.cs
//
//  Original Dll Name: <InstallDir>\SharedContent\Bin\StructDetail\SmartOccurrence\Release\SMPhysConnRules.dll
//  Original Class Name: ‘WebFilletParm’ in VB content
//
//Abstract
//	WebFilletParameterRule is a .NET parameter rule which is defining the custom parameter rule. 
...
namespace Ingr.SP3D.Content.Structure
{
    ...
    [RuleInterface(DetailingCustomAssembliesConstants.IASMPhysConnRules_WebFilletParm, DetailingCustomAssembliesConstants.IASMPhysConnRules_WebFilletParm)]
    [DefaultLocalizer(ConnectionsResourceIdentifiers.ConnectionsResource, ConnectionsResourceIdentifiers.ConnectionsAssembly)]
    public class WebFilletParameterRule : PhysicalConnectionParameterRule
    {
        //==============================================================================================================
        //DefinitionName/ProgID of this symbol is "Connections,Ingr.SP3D.Content.Structure.WebFilletParameterRule"
        //==============================================================================================================
        #region Parameters
        ...
        public override void Evaluate()
        {
            try
            {
                ...
                if (physicalConnection.PartName.Equals(DetailingCustomAssembliesConstants.WebFillet))
                {
                    try
                    {
                ...
                double noseOrientationAngleValue = 0.0, noseMethodValue = 0.0;
                ...
                double moldedFilletValue = 0, antiMoldedFilletValue = 0, webThickness = 0.0;
                ...
                ProfilePart profilePart = base.BoundedObject as ProfilePart;
                if (profilePart != null)
                {
                    webThickness = DetailingCustomAssembliesServices.GetWebThickness(profilePart.CrossSection);
                    if (webThickness <= Math3d.FitTolerance * 8)
                    {
                        moldedFilletValue = Math3d.FitTolerance * 5;
                    }
                    else if (webThickness <= Math3d.FitTolerance * 10)
                    {
                        moldedFilletValue = Math3d.FitTolerance * 6;
                    }
                    else if (webThickness <= Math3d.FitTolerance * 12)
                    {
                        moldedFilletValue = Math3d.FitTolerance * 8;
                    }
                    else if (webThickness <= Math3d.FitTolerance * 15)
                    {
                        moldedFilletValue = Math3d.FitTolerance * 10;
                    }
                    else if (webThickness <= Math3d.FitTolerance * 20)
                    {
                        moldedFilletValue = Math3d.FitTolerance * 12;
                    }
                    else if (webThickness <= Math3d.FitTolerance * 25)
                    {
                        moldedFilletValue = Math3d.FitTolerance * 15;
                    }
                    else
                    {
                        moldedFilletValue = Math3d.FitTolerance * 20;
                    }
                }
                //Override fillets based on table
                antiMoldedFilletValue = moldedFilletValue;

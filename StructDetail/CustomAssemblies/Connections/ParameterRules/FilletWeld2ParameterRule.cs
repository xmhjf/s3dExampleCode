...
//
//File
//  FilletWeld2ParameterRule.cs
//
//  Original Dll Name: <InstallDir>\SharedContent\Bin\StructDetail\SmartOccurrence\Release\SMPhysConnRules.dll
//  Original Class Name: ‘FilletWeld2Parm’ in VB content
//
//Abstract
//	FilletWeld2ParameterRule is a .NET parameter rule which is defining the custom parameter rule. 
...
namespace Ingr.SP3D.Content.Structure
{
    ...
    [RuleInterface(DetailingCustomAssembliesConstants.IASMPhysConnRules_FilletWeld2Parm, DetailingCustomAssembliesConstants.IASMPhysConnRules_FilletWeld2Parm)]
    [DefaultLocalizer(ConnectionsResourceIdentifiers.ConnectionsResource, ConnectionsResourceIdentifiers.ConnectionsAssembly)]
    public class FilletWeld2ParameterRule : PhysicalConnectionParameterRule
    {
        //==============================================================================================================
        //DefinitionName/ProgID of this symbol is "Connections,Ingr.SP3D.Content.Structure.FilletWeld2ParameterRule"
        //==============================================================================================================
        #region Parameters
        ...
        public override void Evaluate()
        {
            try
            {
                ...
                if (physicalConnection.PartName.Equals(DetailingCustomAssembliesConstants.FilletWeld2))
                {
                    try
                    {
                    ...
                    {
                        string s2 = base.GetString(ConnectionsResourceIdentifiers.ErrEndCutType, "Failed to get endcut type");
                        string s1 = s2 + ex.Message + this.GetType().Name + " physicalconnection parameter rule.";
                        base.ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, s1);
                    }
                }
                else
                {
                ...
                switch (bevelMethod)
                {
                    case (int)BevelMethod.Constant:
                        {
                            this.noseOrientationAngle.Value = Math.PI - physicalConnection.GetConnectionAngle();

Attribute VB_Name = "FullyParametricFunPH"
'+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
'
'   Copyright (c) 2003-07, Intergraph Corporation. All rights reserved.
'
'   FullyParametricFunPH.bas
'   Author:
'   Creation Date:
'   Description:
...
'                                    Modified port index logic. Added new function 'CreateRetrieveDynamicNozzlePHForEquipment'
'                                    which can be used for any port index.Modified "CreateRetrieveDynamicEquipmentNozzle" in same way.
'   28.Nov.2004     JFF              Created by duplication and edit from FullyParametricFun for SmartOccurrence
'  08.SEP.2006     KKC  DI-95670  Replace names with initials in all revision history sheets and symbols
'  1.NOV.2007      RRK  CR-123952  Modfied CreateRetrieveDynamicNozzlePH, CreateRetrieveDynamicEquipmentNozzlePH, CreateRetrieveDynamicNozzleForEquipmentPH
'                                  functions to give cptOffset value zero when port termination class is bolted using a new optional boolean
'                                  parameter as argument
'+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Option Explicit

Const MODULE = "FullyParametricFunPH"
...
Public Function CreateRetrieveDynamicNozzlePH(PortIndex As Long, DefNPD As Double, DefNPDUnitType As String, _
    ...
    Const METHOD = "CreateRetrieveDynamicNozzlePH:"
    On Error GoTo ErrorHandler
...
'
    ...
    ''uncomment to make test with text inputs
        Set oNozzle = oNozzlePHFactory.CreatePipeNozzlePH(PortIndex, Npd, NPDUnitType, _
        ...
        Set CreateRetrieveDynamicNozzlePH = oNozzle
        Set oNozzle = Nothing
...
Public Function CreateRetrieveDynamicEquipmentNozzlePH(ByRef oPartFclt As PartFacelets.IJDPart, PortIndex As Long, Npd As Double, _
                    NPDUnitType As String, EndPreparation As Long, ScheduleThickness As Long, _
                    EndStandard As Long, PressureRating As Long, FlowDirection As DistribFlow, _
                    Id As String, objOutputColl As Object, m_oCodeListMetadata As IJDCodeListMetaData, _
                    CreateFlag As Boolean, ByRef lPipeDiam() As Double, ByRef lFlangeThick() As Double, _
    ...
    Const METHOD = "CreateRetrieveDynamicEquipmentNozzlePH:"
...
''''        End If
    ...
    'Return nozzle and other values
        ...
        Set CreateRetrieveDynamicEquipmentNozzlePH = oNozzle
        Set oNozzle = Nothing
    Else
...
'   ii. Array size which is being passed into this function (for eg. lFlangeDiam() etc.)
'       should be same as the number of nozzles created using this function.
Public Function CreateRetrieveDynamicNozzlePHForEquipment(PortIndex As Long, DefNPD As Double, DefNPDUnitType As String, _
                    DefEndPreparation As Long, DefScheduleThickness As Long, DefEndStandard As Long, _
                    DefPressureRating As Long, DefFlowDirection As DistribFlow, Npd As Double, _
                    NPDUnitType As String, EndPreparation As Long, ScheduleThickness As Long, _
                    EndStandard As Long, PressureRating As Long, FlowDirection As DistribFlow, _
    ...
    Const METHOD = "CreateRetrieveDynamicNozzlePHForEquipment:"
    Const E_FAIL = -2147467259
    On Error GoTo ErrorHandler
    Const MAX_NO_NOZ = 30
    Static nozzleCount As Integer
    ...
    ''uncomment to make test with text inputs
        ...
        lDepth(nozzleCount) = depth(nozzleCount)
        lsptoffset(nozzleCount) = sptOffset(nozzleCount)
        Set CreateRetrieveDynamicNozzlePHForEquipment = oNozzle
        Set oNozzle = Nothing
    Else
    ' if the function is called from places other than Physical, only return the values.
        For iCount = 1 To UBound(lPipeDiam)

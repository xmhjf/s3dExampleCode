...
{
    /// <summary>
    /// Default Weld Implements Planning Joint Rule Base Class
    /// </summary>
    public class DefaultWeld : PlanningJointRuleBase
    {
        ...
        public override ReadOnlyCollection<ComplexString3d> GenerateGeometries(BusinessObject physicalConn)
        {
            ...
            try
            {
                ...
                if (weldType == WeldType.TeeWeld)
                {
                    CreateSideWelds = true;
                }

                //weldCategoryName is Note needed  for default Weld case
                geometryCollections = GeneratePlnJointGeometry(WeldGeometry.DefaultWeld, singlePJForMultiLumpPC, weldType, weldCategory, blockIntersectionJointLength);
            }
            catch (Exception e)
            {

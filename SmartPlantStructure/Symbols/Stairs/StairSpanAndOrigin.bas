Attribute VB_Name = "StairSpanAndOrigin"
Option Explicit
...
' 05-May-2007   AS CR#116177 Added new functions for used in Mirror copy support
...
' 02-Sept-2008   GG CR#149875 Added optional argument bAttributeExists for function GetAttribute
' 03-Oct-2008   GG DI#150310 Added Enum eRepresentationType
' 20-Apr-2009   AS TR#163238 Modified code to clean up unnecessary exceptions thrown during synch
' 15-May-2009   AS TR#164843 Replaced if-else with if block as the code is structured around that assumption
'***************************************************************************************

Private Const MODULE = "SPSStairMacros.SetSpanAndOrigin"
...
Public Function SetOriginAndSpan(StairBO As ISPSStair, ByRef Top As Object, _
...
Const METHOD = "SetOriginAndSpan"
    ...
    Dim x As Double, y As Double, z As Double  'Temp Vars
    ...
    'get the normal either from the surface or the plane
    
    'Initialize it to z-vector
    x = 0
    y = 0
    z = 1
    
    'TR-CP-163238  NULL pointer usage during Synchronize
    If Not oBottomSurf Is Nothing Then
    ...
    ElseIf Not oBottomPlane Is Nothing Then
        oBottomPlane.GetNormal x, y, z
    End If
    
    'z-vector is always pointing up to get correct out-vector
    BottomNormal.Set 0, 0, 1
    ...
    If oTopCurve Is Nothing And oTopEdge Is Nothing Then
        If oTopSurf Is Nothing Then
            Err.Clear
            Err.Description = oLocalizer.GetString(IDS_STAIR_MISSING_INPUTS_TOPEDGE, "Stair is missing the top edge reference object. Edit stair and select a new top edge.")
            GoTo ErrorHandler
        End If
    End If
    ...
    If oBottomSurf Is Nothing And oBottomPlane Is Nothing Then
        Err.Clear
        Err.Description = oLocalizer.GetString(IDS_STAIR_MISSING_INPUTS_BOTTOM, "Stair is missing the bottom surface reference object. Edit stair and select a new bottom reference.")
        GoTo ErrorHandler
    End If
    ...
    'If Not BoolArc Then    'First step toward a fix for TR130780, TR124571, TR133869, TR133944 is to retrieve horizontal offset for curved cases.
        dHoriz = StairBO.HorizVertOffset.HorizontalOffset
    'End If
    dVert = StairBO.HorizVertOffset.VerticalOffset
    ...
    If Not proj Is Nothing Or oTopSurf Is Nothing Then
        ...
        'End 01/10/2008 C.C.P. TR130780, TR124571, TR133869, TR133944 Problems with horizontal offset for Stairs/ladders with Curved Member top support.
            StairBO.GetPosition x, y, z
            If x <> position.x Or y <> position.y Or z <> position.z Then
                StairBO.SetPosition position.x, position.y, position.z
            End If
        End If
        ...
        Else
            dHoriz = Mindist
            StairBO.HorizVertOffset.HorizontalOffset = dHoriz
            StairBO.SetPosition position.x, position.y, position.z
        End If
    End If
...
RefEdgeMissingHandler:
    Set oLocalizer = Nothing
    SetOriginAndSpan = STAIR_E_SIDE_REF_MISSING
    Err.Raise STAIR_E_SIDE_REF_MISSING
    Exit Function
    
ErrorHandler:
    'Err.Description = oLocalizer.GetString(IDS_STAIR_MISSING_INPUTS, "Stair has missing inputs")
    SetOriginAndSpan = E_FAIL
    Err.Raise E_FAIL
    Set oLocalizer = Nothing
End Function
...
Public Function GetAttribute(OccAttrs As IJDAttributes, strAttribute As String, InfosColl As IJDInfosCol, Optional ByRef AttributeExists As Boolean)    'this is collection of all interfaces
...
Private Function GetIntersectPoint(oTopEdge As Object, oRefEdge As Object) As DPosition
...
On Error GoTo ErrorHandler
    ...
    'Check what is the reference geometry- line/curve,plane/surface
    If Not oRefEdge Is Nothing Then
        If TypeOf oRefEdge Is IJLine Then
            Set oRefLine = oRefEdge
        End If
        If TypeOf oRefEdge Is IJCurve Then
            Set oRefCurve = oRefEdge
        End If
        If TypeOf oRefEdge Is IJPlane Then
            Set oRefFace = oRefEdge
        End If
        If TypeOf oRefEdge Is IJSurface Then
            Set oRefSurf = oRefEdge
        End If
    End If
    
    'Check what is the top geometry- line/curve,plane/surface
    If Not oTopEdge Is Nothing Then
        If TypeOf oTopEdge Is IJLine Then
            Set oTopLine = oTopEdge
        End If
        If TypeOf oTopEdge Is IJCurve Then
            Set oTopCurve = oTopEdge
        End If
        If TypeOf oTopEdge Is IJPlane Then
            Set oTopface = oTopEdge
        End If
        If TypeOf oTopEdge Is IJSurface Then
            Set oTopSurf = oTopEdge
        End If
    End If
    
    'Cant handle the cae wher both top and side reference are surfaces - Old code
    'should be reviewed and jettisoned at some point.
    If Not oTopSurf Is Nothing And Not oRefSurf Is Nothing Then
        GoTo ErrorHandler
    End If
    
    If oRefEdge Is Nothing Then
        If Not oTopCurve Is Nothing Then
            oTopCurve.EndPoints xSt, ySt, zSt, xEn, yEn, zEn
    ...
    If Not oRefCurve Is Nothing Or oTopSurf Is Nothing And oRefSurf Is Nothing Then
        ...
        If Not oTopCurve Is Nothing Then
            oTopCurve.EndPoints x1, y1, z1, x2, y2, z2
            ...
            End If
        ElseIf Not oTopSurf Is Nothing Then
            oTopSurf.Parameter xSt, ySt, zSt, ParmU, ParmV
            oTopSurf.position ParmU, ParmV, x2, y2, z2
        End If
    ElseIf Not oRefSurf Is Nothing And Not oTopCurve Is Nothing Then
        oTopCurve.EndPoints xSt, ySt, zSt, xEn, yEn, zEn
...
Public Function GetIntersectPointOnSideRef(oStair As ISPSStair) As IJDPosition
...
On Error GoTo ErrorHandler
    ...
    oStair.GetPosition posX, posY, posZ
    oStair.GetMatrix oMatrix
...
Public Function GetDirectionOfTopSupport(oStair As ISPSStair) As IJDVector
...
On Error GoTo ErrorHandler
    ...
    ElseIf TypeOf oTop Is IJSurface Then
        ...
        oStair.GetPosition tempX, tempY, tempZ

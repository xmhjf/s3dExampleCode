...
//
//   Util_Gusset_Metric.cs
//    Util_Metric,Ingr.SP3D.Content.Support.Symbols.Util_Gusset_Metric
//   Author       :  Rajeswari
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    [VariableOutputs]
    public class Util_Gusset_Metric : HangerComponentSymbolDefinition, ICustomHgrBOMDescription, ICustomWeightCG
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "Util_Metric,Ingr.SP3D.Content.Support.Symbols.Util_Gusset_Metric"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Double alpha = Math.Atan(depth / width);
                Double hype = Math.Sqrt(Math.Abs(depth * depth + width * width));

                Double portH = Math.Sin(alpha) * (hype / 2);
                Double portW = Math.Cos(alpha) * (hype / 2);

                Double port2x = T / 2;
                Double port2y = width - portW;
                Double port2z = depth - portH;

                Vector vector = new Vector(0, (port2z / ((port2y * port2y) + (port2z * port2z))), (port2y / ((port2y * port2y) + (port2z * port2z))));
                vector.Length = 1;

                Port port1 = new Port(OccurrenceConnection, part, "CornerStructure", new Position(0, 0, 0), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_Symbolic.Outputs["Port1"] = port1;
                Port port2 = new Port(OccurrenceConnection, part, "EdgeStructure", new Position(0, port2y, port2z), new Vector(1, 0, 0), vector);
            ...
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, Util_MetricLocalizer.GetString(Util_MetricSymbolsResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of Util_Gusset_Metric.cs."));
                return;
            }
        }
        #endregion
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, Util_MetricLocalizer.GetString(Util_MetricSymbolsResourceIDs.ErrBOMDescription, "Error in BOMDescription of Util_Gusset_Metric.cs."));
                return "";
            }
        }
        #endregion
        ...
        void ICustomWeightCG.EvaluateWeightCG(BusinessObject supportComponentBO)
        {
            try
            {
                ...
                try
                {
                    weight = 0.5 * depth * width * T * getSteelDensityKGPerM;
                }
                catch
                {
                    weight = 0;
            ...
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, Util_MetricLocalizer.GetString(Util_MetricSymbolsResourceIDs.ErrWeightCG, "Error in WeightCG of Util_Gusset_Metric.cs."));
            }
        }
        #endregion
    }

...
//
//   SFS5400.cs
//    FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5400
//   Author       : Rajeswari
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class SFS5400 : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5400"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                if (A <= 0 && pipeDiameter <= 0 && C <= 0)
                {
                    ...
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrInvalidAandpipeDiaandCGTZero, "A value, Pipe Diameter and C value should be greater than zero"));
                    return;
                }
                SymbolGeometryHelper symbolGeometryHelper = new SymbolGeometryHelper();

                symbolGeometryHelper.ActivePosition = new Position(-(F / 2 + T), -B / 2, pipeDiameter / 2);
                Projection3d top1 = (Projection3d)symbolGeometryHelper.CreateBox(null, T, B, (A / 2 - pipeDiameter / 2) + C, 9);
                m_Symbolic.Outputs["TOP1"] = top1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(F / 2, -B / 2, pipeDiameter / 2);
                Projection3d top2 = (Projection3d)symbolGeometryHelper.CreateBox(null, T, B, (A / 2 - pipeDiameter / 2) + C, 9);
                m_Symbolic.Outputs["TOP2"] = top2;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(-(F / 2 + T), -B / 2, -L / 2);
                Projection3d bot1 = (Projection3d)symbolGeometryHelper.CreateBox(null, T, B, (A / 2 - pipeDiameter / 2) + C, 9);
                m_Symbolic.Outputs["BOT1"] = bot1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(F / 2, -B / 2, -L / 2);
                Projection3d bot2 = (Projection3d)symbolGeometryHelper.CreateBox(null, T, B, (A / 2 - pipeDiameter / 2) + C, 9);
            ...
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of SFS5400.cs."));
                return;
            }
        }
        #endregion

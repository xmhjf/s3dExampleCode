...
//
//   SFS5393A.cs
//    FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5393A
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class SFS5393A : HangerComponentSymbolDefinition, ICustomHgrBOMDescription
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5393A"
        //----------------------------------------------------------------------------------
        ...
        [InputDouble(13, "F", "F", 0.999999)]
        public InputDouble m_dF;
        [InputDouble(14, "WorkingTrav", "WorkingTrav", 0.999999)]
        public InputDouble m_dWorkingTrav;
        [InputDouble(15, "Blocked", "Blocked", 1)]
        public InputDouble m_oBlocked;
        [InputDouble(16, "Coefficient", "Coefficient", 0.999999)]
        ...
        [SymbolOutput("JOINT1", "JOINT1")]
        [SymbolOutput("JOINT2", "JOINT2")]
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Top", new Position(0, 0, L + F + F), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                symbolGeometryHelper.ActivePosition = new Position(0, 0, -A + F + W);
                ...
                symbolGeometryHelper.ActivePosition = new Position(-T / 2.0, -B / 2.0, -A + F);
                Projection3d lugBox = symbolGeometryHelper.CreateBox(null, T, B, W, 9);
                m_Symbolic.Outputs["LUG"] = lugBox;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(0, 0, -M);
                symbolGeometryHelper.SetOrientation(normal, normal.GetOrthogonalVector());
                Projection3d joint1Cylinder = symbolGeometryHelper.CreateCylinder(null, D / 2.0, M + F);
                m_Symbolic.Outputs["JOINT1"] = joint1Cylinder;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(-B / 2.0, -T / 2.0, L + F + A - W);
                Projection3d lug2Box = symbolGeometryHelper.CreateBox(null, B, T, W, 9);
                m_Symbolic.Outputs["LUG2"] = lug2Box;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(0, 0, L + F);
                symbolGeometryHelper.SetOrientation(normal, normal.GetOrthogonalVector());
                Projection3d joint2Cylinder = symbolGeometryHelper.CreateCylinder(null, D / 2.0, M + F);
                m_Symbolic.Outputs["JOINT2"] = joint2Cylinder;
            }
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of SFS5393A.cs."));
                    return;
                }
            }
        }
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                if (blocked == 1)
                    ...
                    strBlock = "";
                bomString = "Hanger spring SFS 5393 A " + loadClass + " " + Math.Round(H * 1000, 0) + " " + Math.Round(workingTrav * 1000, 0) + " " + strBlock;
                return bomString;
            }
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrBOMDescription, "Error in BOMDescription of SFS5393A"));
                }
                return "";
            }
        }

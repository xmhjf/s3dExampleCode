...
//
//   SFS5371.cs
//    FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5371
//   Author       :   Vijaya
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    [VariableOutputs]
    public class SFS5371 : CustomSymbolDefinition, ICustomHgrBOMDescription
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5371"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Projection3d top1 = (Projection3d)symbolGeometryHelper.CreateBox(null, thickness, width, holeInset + E + (CtoC / 2 - pipeDiameter / 2), 9);
                m_Symbolic.Outputs["TOP1"] = top1;


                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(gap / 2, -width / 2, pipeDiameter / 2);
                Projection3d top2 = (Projection3d)symbolGeometryHelper.CreateBox(null, thickness, width, holeInset + E + (CtoC / 2 - pipeDiameter / 2), 9);
                m_Symbolic.Outputs["TOP2"] = top2;


                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(-(gap / 2 + thickness), -width / 2, -(length - E) / 2);
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(gap / 2, -width / 2, -(length - E) / 2);
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                Vector bottomnormal1 = new Position(boltL / 2, 0, CtoC / 2 + E).Subtract(new Position(-(boltL / 2), 0, CtoC / 2 + E));
                symbolGeometryHelper.ActivePosition = new Position(-(boltL / 2), 0, CtoC / 2 + E);
                symbolGeometryHelper.SetOrientation(bottomnormal, bottomnormal.GetOrthogonalVector());
                Projection3d botBolt1 = symbolGeometryHelper.CreateCylinder(null, boltDiameter / 2, bottomnormal1.Length);
                m_Symbolic.Outputs["BOT_BOLT_1"] = botBolt1;
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                bomDescription = "Pipe clamp B SFS 5371 DN " + s[0];

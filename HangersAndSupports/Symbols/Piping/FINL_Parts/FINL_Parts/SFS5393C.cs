...
//
//   SFS5393C.cs
//    FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5393C
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class SFS5393C : HangerComponentSymbolDefinition, ICustomHgrBOMDescription
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5393C"
        //----------------------------------------------------------------------------------
        ...
        [InputDouble(12, "U", "U", 0.999999)]
        public InputDouble m_dU;
        [InputDouble(13, "K", "K", 0.999999)]
        public InputDouble m_dK;
        [InputDouble(14, "PinDia", "PinDia", 0.999999)]
        public InputDouble m_dPinDia;
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Top", new Position(0, 0, L - pinDia), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                symbolGeometryHelper.ActivePosition = new Position(0, 0, -pinDia / 2.0 - D / 2.0 + U);
                ...
                matrix.Translate(new Vector(0, 0, -pinDia / 2.0 - D / 2.0 + K / 2.0));
                Revolution3d bend = new Revolution3d((new Circle3d(new Position(K / 2.0, 0, 0), new Vector(0, 0, 1), D / 2.0)), new Vector(0, -1, 0), new Position(0, 0, 0), Math.PI, true);
                bend.Transform(matrix);
                m_Symbolic.Outputs["BEND"] = bend;

                matrix = new Matrix4X4();
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1));
                matrix.Translate(new Vector(0, 0, -pinDia / 2.0 + L + D / 2.0 - K / 2.0));
                Revolution3d bend2 = new Revolution3d((new Circle3d(new Position(K / 2.0, 0, 0), new Vector(0, 0, 1), D / 2.0)), new Vector(0, -1, 0), new Position(0, 0, 0), Math.PI, true);
                bend2.Transform(matrix);
                m_Symbolic.Outputs["BEND2"] = bend2;
            }
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of SFS5393C.cs."));
                    return;
                }
            }
        }
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                if (blocked == 1)
                    ...
                    strBlock = "";
                bomString = "Hanger spring SFS 5393 C " + loadClass + " " + Math.Round(H * 1000, 0) + " " + Math.Round(workingTrav * 1000, 0) + " " + strBlock;
                return bomString;
            }
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrBOMDescription, "Error in BOMDescription of SFS5393C"));
                }
                return "";
            }
        }

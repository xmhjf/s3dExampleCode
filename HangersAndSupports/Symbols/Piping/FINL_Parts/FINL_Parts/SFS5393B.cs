...
//
//   SFS5393B.cs
//    FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5393B
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class SFS5393B : HangerComponentSymbolDefinition, ICustomHgrBOMDescription
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "FINL_Parts,Ingr.SP3D.Content.Support.Symbols.SFS5393B"
        //----------------------------------------------------------------------------------
        ...
        [InputDouble(12, "WorkingTrav", "WorkingTrav", 0.999999)]
        public InputDouble m_dWorkingTrav;
        [InputDouble(13, "Blocked", "Blocked", 0.999999)]
        public InputDouble m_oBlocked;
        [InputDouble(14, "Coefficient", "Coefficient", 0.999999)]
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Top", new Position(0, 0, L), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                if (L == 0 && T == 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrInvalidLandT, "L and T values cannot be zero"));
                    return;
                }
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(-B / 2.0, -T / 2.0, L + A - W);
                Projection3d lug2Box = symbolGeometryHelper.CreateBox(null, B, T, W, 9);
                m_Symbolic.Outputs["LUG2"] = lug2Box;

                symbolGeometryHelper = new SymbolGeometryHelper();
                normal = new Position(0, T / 2.0, L).Subtract(new Position(0, -T / 2.0, L));
                symbolGeometryHelper.ActivePosition = new Position(0, T / 2.0, L);
                symbolGeometryHelper.SetOrientation(normal, normal.GetOrthogonalVector());
                Projection3d hole2Cylinder = symbolGeometryHelper.CreateCylinder(null, D / 2.0, normal.Length);
                m_Symbolic.Outputs["HOLE2"] = hole2Cylinder;
            }
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of SFS5393B.cs."));
                    return;
                }
            }
        }
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                if (blocked == 1)
                    ...
                    strBlock = "";
                bomString = "Hanger spring SFS 5393 B " + loadClass + " " + Math.Round(H * 1000, 0) + " " + Math.Round(workingTrav * 1000, 0) + " " + strBlock;
                return bomString;
            }
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, FINL_PartsLocalizer.GetString(FINL_PartsSymbolResourceIDs.ErrBOMDescription, "Error in BOMDescription of SFS5393B"));
                }
                return "";
            }
        }

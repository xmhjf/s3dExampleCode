...
//
//   ZVxBOM.cs
//   Witzenmann,Ingr.SP3D.Content.Support.Symbols.FSGBOM
//   Author       : Vinod  
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class FSGBOM : SmartPartComponentDefinition, ICustomHgrBOMDescription
    {
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                if (lengthValue > 0)
                {
                    int size = Convert.ToInt32((string)((PropertyValueString)catalogPart.GetPropertyValue("IJUAhsSize", "Size")).PropValue);
                    int LGV = (int)((PropertyValueCodelist)oSupportOrComponent.GetPropertyValue("IJUAhsWLGV_FSG", "ULGVFSG")).PropValue;
                    int surfaceFinish = (int)((PropertyValueCodelist)oSupportOrComponent.GetPropertyValue("IJUAhsWZNFinish", "SurfaceFinish")).PropValue;
                    int threadedConnection = (int)((PropertyValueCodelist)oSupportOrComponent.GetPropertyValue("IJUAhsWThrdConnection", "ThrdConnection")).PropValue;
                    int travelStop = (int)((PropertyValueCodelist)oSupportOrComponent.GetPropertyValue("IJUAhsWTravelStop", "TravelStop")).PropValue;
                    double nominalTravel = ((double)((PropertyValueDouble)oSupportOrComponent.GetPropertyValue("IJUAhsWNominalTravel", "NominalTravel")).PropValue);
                    bomDescription = "FSG" + " " + size.ToString("00") + "." + nominalTravel + "." + (lengthValue * 1000).ToString("0000") + "-" + LGV.ToString("000") + "." + travelStop + "." + threadedConnection + "." + surfaceFinish;
                }
            }
            catch//General Unhandled exception 
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, SmartPartLocalizer.GetString(SmartPartSymbolResourceIDs.ErrStrutABOMDescription, "Error in BOMDescription of FSGBOM.cs."));
            }

...
//
//   Utility_USER_VARIABLE_BOX.cs
//    Utility,Ingr.SP3D.Content.Support.Symbols.Utility_USER_VARIABLE_BOX
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class Utility_USER_VARIABLE_BOX : HangerComponentSymbolDefinition, ICustomWeightCG
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "Utility,Ingr.SP3D.Content.Support.Symbols.Utility_USER_VARIABLE_BOX"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                if (length <= 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, UtilityLocalizer.GetString(UtilitySymbolResourceIDs.ErrLengthGTZero, "Length should be greater than zero"));
                    return;
                }
                if (depth <= 0)
                {
                ...
                Projection3d top2 = (Projection3d)symbolGeometryHelper.CreateBox(null, length , depth, width);
            ...
            catch     //General Unhandled exception 
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, UtilityLocalizer.GetString(UtilitySymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of Utility_USER_VARIABLE_BOX"));
                    return;
                }
            }
        }
        ...
        void ICustomWeightCG.EvaluateWeightCG(BusinessObject supportComponentBO)
        {
            try
            {
                ...
                if (supportComponentBO.SupportsInterface("IJOAHgrUtility_VARIABLE_BOX"))
                {
                    try
                    {
                        dryWeight = (double)((PropertyValueDouble)supportComponentBO.GetPropertyValue("IJOAHgrUtility_VARIABLE_BOX", "DryWt")).PropValue;
                    }
                    catch
                    {
                        dryWeight = 0;
                ...
                else
                {
                    ...
                    double L = (double)((PropertyValueDouble)part.GetPropertyValue("IJUAHgrOccLength", "Length")).PropValue;
                    double width = (double)((PropertyValueDouble)supportComponentBO.GetPropertyValue("IJOAHgrUtility_VARIABLE_BOX", "WIDTH")).PropValue;
                    double depth = (double)((PropertyValueDouble)supportComponentBO.GetPropertyValue("IJOAHgrUtility_VARIABLE_BOX", "DEPTH")).PropValue;
            ...
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, UtilityLocalizer.GetString(UtilitySymbolResourceIDs.ErrWeightCG, "Error in WeightCG of Utility_USER_VARIABLE_BOX."));
                }
            }
        }
        #endregion

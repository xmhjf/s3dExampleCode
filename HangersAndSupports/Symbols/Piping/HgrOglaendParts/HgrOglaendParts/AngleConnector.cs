...
//
//   AngleConnector.cs
//   HgrOglaendParts,Ingr.SP3D.Content.Support.Symbols.AngleConnector
//   Author       :  Ramya
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class AngleConnector : CustomSymbolDefinition, ICustomHgrWeightCG
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "HgrOglaendParts,Ingr.SP3D.Content.Support.Symbols.AngleConnector"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port hole1Port = new Port(connection, part, "Hole1", new Position(length / 2 + hport1OffsetX, 0 + hport1OffsetY, 0 + hport1OffsetZ), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_PhysicalAspect.Outputs["Hole1"] = hole1Port;

                Port hole2Port = new Port(connection, part, "Hole2", new Position(length / 2 + hport2OffsetX, flangeWidth / 2 + hport2OffsetY, 0 + hport2OffsetZ), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_PhysicalAspect.Outputs["Hole2"] = hole2Port;

                Port hole3Port = new Port(connection, part, "Hole3", new Position(length / 2 + hport3OffsetX, 0 + hport3OffsetY, webWidth / 2 + hport3OffsetZ), new Vector(1, 0, 0), new Vector(0, 0, 1));
            ...
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ...
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, OglaendPartLocalizer.GetString(OglaendPartSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of AngleConnector.cs " + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name));
                }
            }
        }

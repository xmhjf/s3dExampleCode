...
//
//   PSL_V2_ES.cs
//    PSL,Ingr.SP3D.Content.Support.Symbols.PSL_V2_ES
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_V2_ES : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_V2_ES"
        //----------------------------------------------------------------------------------

        #region "Definition of Inputs"
        [InputCatalogPart(1)]
        public InputCatalogPart m_PartInput;
        [InputDouble(2, "ROD_LEN", "ROD_LEN", 0.999999)]
        public InputDouble ROD_LEN;
        [InputDouble(3, "WORKING_TRAV", "WORKING_TRAV", 0.999999)]
        public InputDouble WORKING_TRAV;
        [InputDouble(4, "DIR", "DIR", 0.999999)]
        public InputDouble DIR;
        [InputDouble(5, "HOT_LOAD", "HOT_LOAD", 0.999999)]
        public InputDouble HOT_LOAD;
        [InputDouble(6, "TURN_A", "TURN_A", 0.999999)]
        public InputDouble TURN_A;
        [InputDouble(7, "ROD_DIA", "ROD_DIA", 0.999999)]
        public InputDouble ROD_DIA;
        [InputDouble(8, "L", "L", 0.999999)]
        public InputDouble L;
        [InputDouble(9, "M", "M", 0.999999)]
        public InputDouble M;
        [InputDouble(10, "D", "D", 0.999999)]
        public InputDouble D;
        [InputDouble(11, "K", "K", 0.999999)]
        public InputDouble K;
        [InputDouble(12, "RTO", "RTO", 0.999999)]
        public InputDouble RTO;
        [InputString(13, "SIZE", "SIZE", "No Value")]
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "InThdRH", new Position(0, 0, -rodLength - preset - turnA / 2 + b), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                if (HgrCompareDoubleService.cmpdbl(turnA , 0)==true)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidTurnANEZ, "TURN_A value cannot be zero"));
                    return;
                }
                double x = k - Math.Sqrt((d * d) - (m / 2.0) * (m / 2.0));

                Collection<Position> pointCollection = new Collection<Position>();
                pointCollection.Add(new Position(m / 2.0, -k / 2.0 + x, 0));
                pointCollection.Add(new Position(m / 2.0, k / 2.0 - x, 0));
                pointCollection.Add(new Position(0, k / 2.0, 0));
                pointCollection.Add(new Position(-m / 2.0, k / 2.0 - x, 0));
                pointCollection.Add(new Position(-m / 2.0, -k / 2.0 + x, 0));
                pointCollection.Add(new Position(0, -k / 2.0, 0));
                pointCollection.Add(new Position(m / 2.0, -k / 2.0 + x, 0));
                ...
                matrix.Translate(new Vector(0, 0, -rodLength - preset));
                Projection3d rodCylinder = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2, rodLength + rto);
                ...
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                matrix.Translate(new Vector(0, 0, -rodLength - preset - turnA + b));
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                Projection3d turnBCylinder = symbolGeometryHelper.CreateCylinder(null, rodDiameter, turnA);
                turnBCylinder.Transform(matrix);
                m_Symbolic.Outputs["TURNB"] = turnBCylinder;
            }
            catch  //General Unhandled exception 
            {
                if (base.ToDoListMessage == null)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_V2_ES."));
                    return;
                }
            }
        }

...
//
//   PSL_V3_TS3.cs
//    PSL,Ingr.SP3D.Content.Support.Symbols.PSL_V3_TS3
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_V3_TS3 : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_V3_TS3"
        //----------------------------------------------------------------------------------
        ...
        [InputDouble(14, "E", "E", 0.999999)]
        public InputDouble E;
        [InputDouble(15, "G", "G", 0.999999)]
        public InputDouble G;
        [InputString(16, "SIZE", "SIZE", "No Value")]
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Hole", new Position(0, 0, l + g + preset), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                if ((g + f) <= 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidGandFGTZ, "(G + F) value should be greater than zero"));
                    return;
                }
                double x = k - Math.Sqrt((d * d) - (m / 2.0) * (m / 2.0));

                Collection<Position> pointCollection = new Collection<Position>();
                pointCollection.Add(new Position(m / 2.0, -k / 2.0 + x, rto - l + preset - g));
                pointCollection.Add(new Position(m / 2.0, k / 2.0 - x, rto - l + preset - g));
                pointCollection.Add(new Position(0, k / 2.0, rto - l + preset - g));
                pointCollection.Add(new Position(-m / 2.0, k / 2.0 - x, rto - l + preset - g));
                pointCollection.Add(new Position(-m / 2.0, -k / 2.0 + x, rto - l + preset - g));
                pointCollection.Add(new Position(0, -k / 2.0, rto - l + preset - g));
                pointCollection.Add(new Position(m / 2.0, -k / 2.0 + x, rto - l + preset - g));
                ...
                matrix.Translate(new Vector(0, 0, rto - l + preset - g));
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                Projection3d cylinder1 = symbolGeometryHelper.CreateCylinder(null, rodDiameter, (rto - l - g) + 0.077 / 2 + b + preset);
                cylinder1.Transform(matrix);
                m_Symbolic.Outputs["CYLINDER1"] = cylinder1;

                matrix = new Matrix4X4();
                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.Translate(new Vector(e / 2.0, -h, rto - g + preset));
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                Projection3d lugBox = symbolGeometryHelper.CreateBox(null, t, h * 2.0, g + f, 9);
                lugBox.Transform(matrix);
                m_Symbolic.Outputs["LUG"] = lugBox;

                matrix = new Matrix4X4();
                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.Translate(new Vector(-t - e / 2.0, -h, rto - g + preset));
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                Projection3d lug2Box = symbolGeometryHelper.CreateBox(null, t, h * 2.0, g + f, 9);
            ...
            catch  //General Unhandled exception 
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_V3_TS3."));
                    return;
                }
            }
        }

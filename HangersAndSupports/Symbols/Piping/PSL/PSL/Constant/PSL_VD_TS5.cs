...
//
//   PSL_VD_TS5.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VD_TS5
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    /// <summary>
    /// Implementation of the PSL_VD_TS5 class.
    /// </summary>
    /// <remarks></remarks>
    [CacheOption(CacheOptionType.Cached)]
    public class PSL_VD_TS5 : HangerComponentSymbolDefinition
    {
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VD_TS5"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                double baseX = Math.Cos(angle2 * Math.PI / 180) * hypo;
                double baseY = Math.Sin(angle2 * Math.PI / 180) * hypo;
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Structure", new Position(0, D + FG - (FG + U) / 2.0, A2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                if (FK <= 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidFKGTZ, "FK value should be greater than zero"));
                    return;
                }
                if (FM <= 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidFMGTZ, "FM value should be greater than zero"));
                    return;
                }
                ...
                Projection3d trunB = symbolGeometryHelper.CreateCylinder(null, rodDiameter, cylinderLength);
                m_Symbolic.Outputs["TURNB"] = trunB;
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d box = symbolGeometryHelper.CreateBox(null, FJ * 2.0 + U + FG, FK, FM, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - U - FJ, -FK / 2.0, A2 - FM));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VD_TS5"));
                    return;
                }
            }
        }

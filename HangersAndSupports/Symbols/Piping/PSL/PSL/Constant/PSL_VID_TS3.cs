...
//
//   PSL_VID_TS3.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VID_TS3
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_VID_TS3 : HangerComponentSymbolDefinition
    {
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VID_TS3"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                Projection3d rod = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2, J - SB - TJ + E - TempNumber);
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug1 = symbolGeometryHelper.CreateBox(null, FG + U + CC, EE, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - U - CC / 2.0, -GG / 2.0 - EE, A3));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug1.Transform(matrix);
                m_Symbolic.Outputs["LUG1"] = lug1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug3 = symbolGeometryHelper.CreateBox(null, FG + U + CC, EE, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - U - CC / 2.0, GG / 2.0, A3));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug3.Transform(matrix);
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VID_TS3"));
                    return;
                }
            }
        }

...
//
//   PSL_HD_TS3.cs
//   PROGID : PSL,Ingr.SP3D.Content.Support.Symbols.PSL_HD_TS3
//   Author       :  Mahanth
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_HD_TS3 : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_HD_TS3"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Hole", new Position(0, d - u, a1 + aa), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_Symbolic.Outputs["Port2"] = port2;
                Port port3 = new Port(OccurrenceConnection, part, "Hole", new Position(0, d + fg, a1 + aa), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(d - u - cc / 2.0, -gg / 2.0 - ee, a1);
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(d + fg - cc / 2.0, -gg / 2.0 - ee, a1);
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(d - u - cc / 2.0, gg / 2.0, a1);
                Projection3d lug3 = symbolGeometryHelper.CreateBox(null, cc, ee, aa + bb, 9);
                rotateMatrix.SetIdentity();
                rotateMatrix.Rotate(Math.PI / 2, new Vector(0, 0, 1));
                lug3.Transform(rotateMatrix);
                m_Symbolic.Outputs["LUG3"] = lug3;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(d + fg - cc / 2.0, gg / 2.0, a1);
                Projection3d lug4 = symbolGeometryHelper.CreateBox(null, cc, ee, aa + bb, 9);
                rotateMatrix.SetIdentity();
                rotateMatrix.Rotate(Math.PI / 2, new Vector(0, 0, 1));
                lug4.Transform(rotateMatrix);
                m_Symbolic.Outputs["LUG4"] = lug4;
            }
            catch (Exception)
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_HD_TS3.cs."));
                return;
            }
        }

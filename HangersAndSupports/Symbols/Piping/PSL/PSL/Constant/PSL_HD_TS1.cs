...
//
//   PSL_HD_TS1.cs
//   PROGID : PSL,Ingr.SP3D.Content.Support.Symbols.PSL_HD_TS1
//   Author       :  Mahanth
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_HD_TS1 : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_HD_TS1"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "InThdRH", new Position(0, d - u, a1 - xn / 2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_Symbolic.Outputs["Port2"] = port2;
                Port port3 = new Port(OccurrenceConnection, part, "InThdRH", new Position(0, d + fg, a1 - xn / 2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                rotateMatrix.SetIdentity();
                rotateMatrix.Rotate(angle * Math.PI / 180, new Vector(0, 1, 0), new Position(0, 0, 0));
                ...
                Collection<Position> pointCollection1 = new Collection<Position>();
                pointCollection1.Add(new Position(-fb / 2.0, d - tc, a1 - fm));
                pointCollection1.Add(new Position(-fb / 2.0, d + fc, a1 - fm));
                pointCollection1.Add(new Position(-fb / 2.0, d + fc, a1 - tb - fp));
                pointCollection1.Add(new Position(-fb / 2.0, d - location, a1 - tb - fp));
                pointCollection1.Add(new Position(-fb / 2.0, d - location, a1 - tb + location));
                pointCollection1.Add(new Position(-fb / 2.0, d - tc, a1 - rodDiameter * 2.0));
                pointCollection1.Add(new Position(-fb / 2.0, d - tc, a1 - fm));
                Projection3d body = new Projection3d(new LineString3d(pointCollection1), new Vector(1, 0, 0), fb, true);
                m_Symbolic.Outputs["BODY"] = body;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                Projection3d cyl1 = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2.0, xn);
                rotateMatrix.SetIdentity();
                rotateMatrix.Translate(new Vector(d - u, 0, a1 - xn));
                rotateMatrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                cyl1.Transform(rotateMatrix);
                m_Symbolic.Outputs["CYL1"] = cyl1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                Projection3d cyl2 = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2.0, xn);
                rotateMatrix.SetIdentity();
                rotateMatrix.Translate(new Vector(d + fg, 0, a1 - xn));
                rotateMatrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                cyl2.Transform(rotateMatrix);
                m_Symbolic.Outputs["CYL2"] = cyl2;                
            }
            catch (Exception)
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_HD_TS1.cs."));
                return;
            }
        }

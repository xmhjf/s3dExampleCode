...
//
//   PSL_VID_TS4.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VID_TS4
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_VID_TS4 : HangerComponentSymbolDefinition
    {
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VID_TS4"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                double calculatedTotalTravel = 10.0 * (int)(moment * 100.0 / operLoad + 0.5);
                ...
                double SB = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "SB", parameter);
                ...
                double TJ = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "TJ", parameter);
                ...
                double FN = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "FN", parameter);
                ...
                double V3 = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "V3", parameter);
                ...
                double FM = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "FM", parameter);
                double J = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "A3", parameter);

                double A3 = J + setPosition;
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Hole", new Position(0, D - U, A3 + AA), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_Symbolic.Outputs["Port2"] = port2;
                Port port3 = new Port(OccurrenceConnection, part, "Hole", new Position(0, D + FG, A3 + AA), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                if (XQ <= 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidXQGTZ, "XQ value should be greater than zero"));
                    return;
                }
                if (EE <= 0)
                {
                ...
                if (HgrCompareDoubleService.cmpdbl(SB , 0 ) == true)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidSBNEZ, "SB value cannot be zero"));
                    return;
                }
                ...
                matrix.Translate(new Vector(0, 0, -locZ));
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                turnb.Transform(matrix);
                ...
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                Projection3d rod = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2.0, J - SB - TJ + E - tempNumber);
                ...
                matrix.Translate(new Vector(D, -rodDiameter, A3 - SB - TJ));
                ...
                pointCollection.Add(new Position(0, D - FD + FE / 2.0, A3 - SB));
                pointCollection.Add(new Position(-FA / 2.0, D - FD + FE * 0.125, A3 - SB));
                pointCollection.Add(new Position(-FA / 2.0, D - FD - FE * 0.125, A3 - SB));
                pointCollection.Add(new Position(0, D - FD - FE / 2.0, A3 - SB));
                pointCollection.Add(new Position(FA / 2.0, D - FD - FE * 0.125, A3 - SB));
                pointCollection.Add(new Position(FA / 2.0, D - FD + FE * 0.125, A3 - SB));
                pointCollection.Add(new Position(0, D - FD + FE / 2.0, A3 - SB));
                Projection3d baseCollection = new Projection3d(new LineString3d(pointCollection), new Vector(0, 0, 1), SB, true);
                m_Symbolic.Outputs["BASE"] = baseCollection;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d body = symbolGeometryHelper.CreateBox(null, FE / 2.0 + FD + FN, XQ, TJ + FP, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - FD - FE / 2.0, -XQ / 2.0, A3 - SB - TJ - FP));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                body.Transform(matrix);
                m_Symbolic.Outputs["BODY"] = body;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug1 = symbolGeometryHelper.CreateBox(null, EE, CC, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - U - EE / 2.0, -CC / 2.0, A3));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug1.Transform(matrix);
                m_Symbolic.Outputs["LUG1"] = lug1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug2 = symbolGeometryHelper.CreateBox(null, EE, CC, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D + FG - EE / 2.0, -CC / 2.0, A3));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug2.Transform(matrix);
                m_Symbolic.Outputs["LUG2"] = lug2;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d plate = symbolGeometryHelper.CreateBox(null, U + FG + EE, CC, EE, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - U - EE / 2.0, -CC / 2.0, A3 - EE));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VID_TS4"));
                    return;
                }
            }
        }

...
//
//   PSL_VIS_TS3.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VIS_TS3
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_VIS_TS3 : HangerComponentSymbolDefinition
    {

        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VIS_TS3"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Hole", new Position(0, D - V3, A3 + AA), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(-TA, 0, -rodDiameter * 2.0));
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug1 = symbolGeometryHelper.CreateBox(null, CC + U, EE, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - V3 - CC / 2.0, -GG / 2.0 - EE, A3));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug1.Transform(matrix);
                m_Symbolic.Outputs["LUG1"] = lug1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug2 = symbolGeometryHelper.CreateBox(null, CC + U, EE, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - V3 - CC / 2.0, GG / 2.0, A3));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VIS_TS3"));
                    return;
                }
            }
        }

...
//
//   PSL_VD_TS1.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VD_TS1
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    /// <summary>
    /// Implementation of the PSL_VD_TS1 class.
    /// </summary>
    /// <remarks></remarks>
    [CacheOption(CacheOptionType.Cached)]
    public class PSL_VD_TS1 : HangerComponentSymbolDefinition
    {

        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VD_TS1"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "InThdRH", new Position(0, D - U, A2 - XN / 2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_Symbolic.Outputs["Port2"] = port2;
                Port port3 = new Port(OccurrenceConnection, part, "InThdRH", new Position(0, D + FG, A2 - XN / 2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                if (HgrCompareDoubleService.cmpdbl(XN, 0) == true)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrInvalidXNNEZ, "XN value should be greater than zero"));
                    return;
                }
                ...
                matrix.Translate(new Vector(D - U, 0, A2 - XN));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                Projection3d cylinder1 = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2, XN);
                cylinder1.Transform(matrix);
                m_Symbolic.Outputs["CYL1"] = cylinder1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D + FG, 0, A2 - XN));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                symbolGeometryHelper.SetOrientation(new Vector(0, 0, 1), new Vector(0, 0, 1).GetOrthogonalVector());
                Projection3d cylinder2 = symbolGeometryHelper.CreateCylinder(null, rodDiameter / 2, XN);
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VD_TS1"));
                    return;
                }
            }
        }

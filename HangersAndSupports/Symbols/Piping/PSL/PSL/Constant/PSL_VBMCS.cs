...
//
//   PSL_VBMCS.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VBMCS
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_VBMCS : HangerComponentSymbolDefinition
    {

        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VBMCS"

        #region "Definition of Inputs"
        [InputCatalogPart(1)]
        public InputCatalogPart PartInput;
        [InputDouble(2, "WORKING_TRAV_DOWN", "WORKING_TRAV_DOWN", 0.999999)]
        public InputDouble WORKING_TRAV_DOWN;
        [InputDouble(3, "WORKING_TRAV_UP", "WORKING_TRAV_UP", 0.999999)]
        public InputDouble WORKING_TRAV_UP;
        [InputDouble(4, "TOTAL_TRAV1", "TOTAL_TRAV1", 0.999999)]
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                double J = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "J7", parameter);
                ...
                double FV = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "FV", parameter);
                double FY = (double)PSLSymbolServices.GetDataByMultipleConditions("PSL_CONSTANTS", "IJUAHgrPSL_CONSTANTS", "FY", parameter);

                double J7 = J - setPosition;
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Structure", new Position(0, D + (XB + XC) / 2.0 - XB, -J7), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                matrix.Translate(new Vector(-FQ / 2.0, -FQ / 2.0, -FF));
                ...
                symbolGeometryHelper.ActivePosition = new Position(0, 0, 0);
                Projection3d bar = symbolGeometryHelper.CreateBox(null, FQ / 2.0, FQ / 2.0, J7 - SE - E - FF, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(-FQ / 4.0, -FQ / 4.0, -FF - (J7 - SE - E - FF)));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                bar.Transform(matrix);
                m_Symbolic.Outputs["BAR"] = bar;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(0, 0, 0);
                Projection3d arm1 = symbolGeometryHelper.CreateBox(null, C, FQ / 4.0, FQ / 4, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(-C, 0, -FQ / 8.0));
                matrix.Rotate(angle * Math.PI / 180.0, new Vector(0, 1, 0), new Position(0, 0, 0));
                matrix.Translate(new Vector(D, -FQ / 8.0, -J7 + SE + FY));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                arm1.Transform(matrix);
                m_Symbolic.Outputs["ARM1"] = arm1;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(0, 0, 0);
                Projection3d arm2 = symbolGeometryHelper.CreateBox(null, TA, FQ / 2.0, FQ / 2, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(-TA, 0, -FQ / 4));
                matrix.Rotate(angle * Math.PI / 180.0, new Vector(0, 1, 0), new Position(0, 0, 0));
                matrix.Translate(new Vector(D, -FQ / 4.0, -J7 + SE));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                arm2.Transform(matrix);
                m_Symbolic.Outputs["ARM2"] = arm2;

                Collection<Position> pointCollection = new Collection<Position>();
                pointCollection.Add(new Position(0, D - rodDiamater, -(J7 - SE + FC) + FF));
                pointCollection.Add(new Position(-FA / 2.0, D - rodDiamater + FE * 0.375, -(J7 - SE + FC) + FF));
                pointCollection.Add(new Position(-FA / 2.0, D - rodDiamater + FE * 0.625, -(J7 - SE + FC) + FF));
                pointCollection.Add(new Position(0, D - rodDiamater + FE, -(J7 - SE + FC) + FF));
                pointCollection.Add(new Position(FA / 2.0, D - rodDiamater + FE * 0.625, -(J7 - SE + FC) + FF));
                pointCollection.Add(new Position(FA / 2.0, D - rodDiamater + FE * 0.375, -(J7 - SE + FC) + FF));
                pointCollection.Add(new Position(0, D - rodDiamater, -(J7 - SE + FC) + FF));
                Projection3d baseProjection = new Projection3d(new LineString3d(pointCollection), new Vector(0, 0, -1), (SE - FC - FF) + FF, true);
                m_Symbolic.Outputs["BASE"] = baseProjection;

                symbolGeometryHelper = new SymbolGeometryHelper();
                symbolGeometryHelper.ActivePosition = new Position(0, 0, 0);
                Projection3d body = symbolGeometryHelper.CreateBox(null, FE, FB, FY + FC + 0.05, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - rodDiamater, -FB / 2.0, -(J7 - SE - FV - FQ / 2.0) - (FC + FV + FQ / 2.0) + FF));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                ...
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - XB + (XB + XC) / 2.0 - XA / 2.0, -FZ / 2.0, -J7));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VBMCS"));
                    return;
                }
            }
        }

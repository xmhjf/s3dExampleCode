...
//
//   PSL_VD_TS3.cs
//   PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VD_TS3
//   Author       :BS   
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    /// <summary>
    /// Implementation of the PSL_VD_TS3 class.
    /// </summary>
    /// <remarks></remarks>
    [CacheOption(CacheOptionType.Cached)]
    public class PSL_VD_TS3 : HangerComponentSymbolDefinition
    {
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_VD_TS3"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port2 = new Port(OccurrenceConnection, part, "Hole", new Position(0, D - U, A2 + AA + d / 2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                m_Symbolic.Outputs["Port2"] = port2;
                Port port3 = new Port(OccurrenceConnection, part, "Hole", new Position(0, D + FG, A2 + AA + d / 2), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                matrix.Translate(new Vector(D - U - CC / 2.0, -GG / 2.0 - EE, A2));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                ...
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D + FG - CC / 2.0, -GG / 2.0 - EE, A2));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug2.Transform(matrix);
                m_Symbolic.Outputs["LUG2"] = lug2;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug3 = symbolGeometryHelper.CreateBox(null, CC, EE, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D - U - CC / 2.0, GG / 2.0, A2));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug3.Transform(matrix);
                m_Symbolic.Outputs["LUG3"] = lug3;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Projection3d lug4 = symbolGeometryHelper.CreateBox(null, CC, EE, AA + BB, 9);
                matrix = new Matrix4X4();
                matrix.Translate(new Vector(D + FG - CC / 2.0, GG / 2.0, A2));
                matrix.Rotate(Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                lug4.Transform(matrix);
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_VD_TS3"));
                    return;
                }
            }
        }

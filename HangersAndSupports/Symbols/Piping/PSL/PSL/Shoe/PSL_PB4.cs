...
//
//   PSL_PB4.cs
//    PSL,Ingr.SP3D.Content.Support.Symbols.PSL_PB4
//   Author       :  Vijaya
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_PB4 : HangerComponentSymbolDefinition
    {

        //DefinitionName/ProgID of this symbol is "PSL_SHOE,Ingr.SP3D.Content.Support.Symbols.PSL_PB4"
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                Port port5 = new Port(OccurrenceConnection, part, "Structure", new Position(0, -c, 0), new Vector(1, 0, 0), new Vector(0, 0, 1));
                ...
                m_Symbolic.Outputs["EP2_1"] = ep2;

                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.SetIdentity();
                symbolGeometryHelper.ActivePosition = new Position(-a / 2, -e / 2 + clampE / 2 - endThickness / 2, -c);
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d ep11 = symbolGeometryHelper.CreateBox(null, a, endThickness, c - pipeDiameter / 2 - clampF, 9);
                ep11.Transform(matrix);
                m_Symbolic.Outputs["EP1_2"] = ep11;

                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.SetIdentity();
                symbolGeometryHelper.ActivePosition = new Position(-a / 2, e / 2 - clampE / 2 - endThickness / 2, -c);
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d ep21 = symbolGeometryHelper.CreateBox(null, a, endThickness, c - pipeDiameter / 2 - clampF, 9);
                ep21.Transform(matrix);
                m_Symbolic.Outputs["EP2_2"] = ep21;
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.SetIdentity();
                symbolGeometryHelper.ActivePosition = new Position(-a / 2, -(e - clampE - webThickness) / 2, c - flangeThickness);
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.SetIdentity();
                symbolGeometryHelper.ActivePosition = new Position(-webThickness / 2, -(e - clampE - webThickness) / 2, c - flangeThickness - ((c - pipeDiameter / 2 - flangeThickness) * 0.75));
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                ...
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d basePlate1 = symbolGeometryHelper.CreateBox(null, a, e - clampE - webThickness, flangeThickness, 9);
                ...
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d web2 = symbolGeometryHelper.CreateBox(null, webThickness, e - clampE - webThickness, (c - pipeDiameter / 2 - flangeThickness) * 0.75, 9);
                web2.Transform(matrix);
                m_Symbolic.Outputs["WEB_2"] = web2;
                ...
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d web3 = symbolGeometryHelper.CreateBox(null, (c - pipeDiameter / 2 - flangeThickness) * 0.75, e - clampE - webThickness, webThickness, 9);
                web3.Transform(matrix);
                m_Symbolic.Outputs["WEB_3"] = web3;

                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.SetIdentity();
                symbolGeometryHelper.ActivePosition = new Position(-c, -(e - clampE - webThickness) / 2, -a / 2);
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d basePlate3 = symbolGeometryHelper.CreateBox(null, flangeThickness, e - clampE - webThickness, a, 9);
                basePlate3.Transform(matrix);
                m_Symbolic.Outputs["BASEPLATE_4"] = basePlate3;

                symbolGeometryHelper = new SymbolGeometryHelper();
                matrix.SetIdentity();
                symbolGeometryHelper.ActivePosition = new Position(-c + flangeThickness, -(e - clampE - webThickness) / 2, -webThickness / 2);
                matrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                Projection3d web4 = symbolGeometryHelper.CreateBox(null, (c - pipeDiameter / 2 - flangeThickness) * 0.75, e - clampE - webThickness, webThickness, 9);
                web4.Transform(matrix);
                m_Symbolic.Outputs["WEB_4"] = web4;
            ...
            catch
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_PB4.cs"));
                    return;
                }

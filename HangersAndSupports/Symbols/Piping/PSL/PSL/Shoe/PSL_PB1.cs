...
//
//   PSL_PB1.cs
//    PSL,Ingr.SP3D.Content.Support.Symbols.PSL_PB1
//   Author       :  Manikanth
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class PSL_PB1 : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "PSL,Ingr.SP3D.Content.Support.Symbols.PSL_PB1"
        //----------------------------------------------------------------------------------
        #region "Definition of Inputs"

        [InputCatalogPart(1)]
        public InputCatalogPart PartInput;
        [InputDouble(2, "FLANGE_T", "FLANGE_T", 0.999999)]
        public InputDouble FLANGE_T;
        [InputDouble(3, "END_T", "END_T", 0.999999)]
        public InputDouble END_T;
        [InputDouble(4, "PIPE_DIA", "PIPE_DIA", 0.999999)]
        public InputDouble PIPE_DIA;
        [InputString(5, "CLAMP_SIZE", "CLAMP_SIZE", "No Value")]
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                rotateMatrix.Translate(new Vector(-a / 2.0, -(e - clampE - webThickness) / 2.0, -c));
                ...
                rotateMatrix.Translate(new Vector(-webThickness / 2.0, -(e - clampE - webThickness) / 2.0, -c + flangeThickness));
                rotateMatrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                Projection3d webBox = symbolGeometryHelper.CreateBox(null, webThickness, e - clampE - webThickness, (c - pipeDiameter / 2.0 - flangeThickness) * 0.75, 9);
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                rotateMatrix = new Matrix4X4();
                rotateMatrix.Translate(new Vector(pipeDiameter / 2.0, -e / 2.0, -(clampB + clampF * 2.0) / 2.0));
                rotateMatrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                rotateMatrix = new Matrix4X4();
                rotateMatrix.Translate(new Vector(-d / 2.0, -e / 2.0, -(clampB + clampF * 2.0) / 2.0));
                rotateMatrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
                ...
                symbolGeometryHelper = new SymbolGeometryHelper();
                rotateMatrix = new Matrix4X4();
                rotateMatrix.Translate(new Vector(-d / 2.0, e / 2.0 - clampE, -(clampB + clampF * 2.0) / 2.0));
                rotateMatrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1), new Position(0, 0, 0));
            ...
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, PSLLocalizer.GetString(PSLSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of PSL_PB1.cs."));
                return;
            }
        }

...
//
//   Anvil_FIG86.cs
//   Anvil,Ingr.SP3D.Content.Support.Symbols.Anvil_FIG86
//   Author       :  Rajeswari
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class Anvil_FIG86 : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "Anvil,Ingr.SP3D.Content.Support.Symbols.Anvil_FIG86"
        //----------------------------------------------------------------------------------
        ...
        [InputDouble(2, "CLIP", "CLIP", 1)]
        public InputDouble m_oCLIP;
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                if (C == 0)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, AnvilLocalizer.GetString(AnvilSymbolResourceIDs.ErrInvalidCNZero, "C value cannot be zero"));
                    return;
                }
                ...
                pointCollection.Add(new Position(-A, -A, C / 2 + const_2));
                pointCollection.Add(new Position(-A, H, C / 2 + const_2));
                pointCollection.Add(new Position(-A, H, const_2 - C / 2));
                pointCollection.Add(new Position(-A, -A, const_2 - C / 2));
                pointCollection.Add(new Position(-A, -A, 0));
                ...
                Projection3d bolt = symbolGeometryHelper.CreateCylinder(null, A / 2, 1.1 * C);
                rotateMatrix.Rotate(3 * Math.PI / 2, new Vector(0, 0, 1));
                bolt.Transform(rotateMatrix);
                m_Symbolic.Outputs["BOLT"] = bolt;

                symbolGeometryHelper = new SymbolGeometryHelper();
                Vector normal = new Position(0, 0, C / 2 + const_2 + A).Subtract(new Position(0, 0, C / 2 + const_2));
                symbolGeometryHelper.ActivePosition = new Position(0, 0, C / 2 + const_2);
            ...
            {
                ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, AnvilLocalizer.GetString(AnvilSymbolResourceIDs.ErrConstructOutputs, "Error in constructOutputs of Anvil_FIG86"));
                return;
            }
        }
        #endregion

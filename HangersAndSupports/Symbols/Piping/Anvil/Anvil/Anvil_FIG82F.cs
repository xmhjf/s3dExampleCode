...
//
//   Anvil_FIG82F.cs
//   Anvil,Ingr.SP3D.Content.Support.Symbols.Anvil_FIG82F
//   Author       :  Hema
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    public class Anvil_FIG82F : HangerComponentSymbolDefinition
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "Anvil,Ingr.SP3D.Content.Support.Symbols.Anvil_FIG82F"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                CodelistItem codelist = ((PropertyValueCodelist)part.GetPropertyValue("IJOAHgrAnvil_FIG82F", "DIR")).PropertyInfo.CodeListInfo.GetCodelistItem((int)m_oDIR.Value);
                ...
                PartClass anvilfig82FParts = (PartClass)catalogBaseHelper.GetPartClass("Anvil_SPRING_RATE");
                ReadOnlyCollection<BusinessObject> fig82FParts, fig82FSpringRollerStrokeParts;
                fig82FParts = anvilfig82FParts.GetRelationship("PartClassContainsClassItems", "ClassItem").TargetObjects;
                foreach (BusinessObject part1 in fig82FParts)
                {
                    if (((string)((PropertyValueString)part1.GetPropertyValue("IJUAHgrAnvil_SPRING_RATE", "SIZE")).PropValue == m_oSIZE.Value))
                    {
                        springRate = (double)((PropertyValueDouble)part1.GetPropertyValue("IJUAHgrAnvil_SPRING_RATE", "RATE_" + "FIG82F".Substring(3, 2) + "")).PropValue;
                ...
                PartClass anvilfig82FSpringRollerParts = (PartClass)catalogBaseHelper.GetPartClass("Anvil_FSPRING_ROLLER");
                fig82FSpringRollerStrokeParts = anvilfig82FSpringRollerParts.GetRelationship("PartClassContainsClassItems", "ClassItem").TargetObjects;
                foreach (BusinessObject part1 in fig82FSpringRollerStrokeParts)
                {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, AnvilLocalizer.GetString(AnvilSymbolResourceIDs.ErrInvalidAPipeDiaPGTZero, "Pipe diameter,A and P values should be greater than zero"));
                    return;
                }
            ...
            catch       //General Unhandled exception 
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, AnvilLocalizer.GetString(AnvilSymbolResourceIDs.ErrConstructOutputs, "Error in constructOutputs of Anvil_FIG82F"));
                    return;
                }
            }
        }

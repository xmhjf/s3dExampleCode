...
//
//   ConstantTypeE.cs
//    HSSmartPart,Ingr.SP3D.Content.Support.Symbols.ConstantTypeE
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    [VariableOutputs]
    public class ConstantTypeE : SmartPartComponentDefinition, ICustomWeightCG
    {
        //----------------------------------------------------------------------------------
        //DefinitionName/ProgID of this symbol is "HSSmartPart,Ingr.SP3D.Content.Support.Symbols.ConstantTypeE"
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                {
                    case 1:
                        constant.length1 = constant.offset2 + constant.length2 * Math.Sin(leverAngle) + constant.pivotOffset + rodTakeout;
                        break;
                    case 2:
                        constant.length1 = constant.length - constant.plate3.thickness1 - constant.frame.length1 + constant.pivotOffset + constant.length2 * Math.Sin(leverAngle) + constant.offset2 + rodTakeout;
                        break;
                    case 3:
                        constant.length1 = constant.offset2 + constant.length2 * Math.Sin(leverAngle) - constant.caseCLOffset - constant.casing.ShapeWidth1 / 2 - constant.frameOffset + constant.frame.length1 + constant.plate4.thickness1 + constant.plate3.thickness1 + rodTakeout;
                        break;
                }
                ...
                switch (config)
                {
                    case 1:     //Vertical Casing Above Frame
                        port = new Port(OccurrenceConnection, part, "Structure", new Position(constant.caseCLOffset - constant.topOffset, 0, -constant.pivotOffset), new Vector(1, 0, 0), new Vector(0, 0, 1));
                    ...
                    case 2:     //Vertical Casing Below Frame
                        port = new Port(OccurrenceConnection, part, "Structure", new Position(-constant.topOffset, 0, -constant.pivotOffset + constant.frame.length1 + constant.plate3.thickness1 - constant.length), new Vector(1, 0, 0), new Vector(0, 0, 1));
                        m_PhysicalAspect.Outputs["Structure"] = port;
                    ...
                    case 3:     //Horizontal Casing Beside Frame
                        port = new Port(OccurrenceConnection, part, "Structure", new Position(-constant.topOffset, 0, constant.caseCLOffset + constant.casing.ShapeWidth1 / 2 + constant.frameOffset - constant.plate4.thickness1 - constant.frame.length1 - constant.plate3.thickness1), new Vector(1, 0, 0), new Vector(0, 0, 1));
                        m_PhysicalAspect.Outputs["Structure"] = port;
            ...
            catch //General Unhandled exception 
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, SmartPartLocalizer.GetString(SmartPartSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of TypeE."));
                    return;
                }
            }
        }
        ...
        void ICustomWeightCG.EvaluateWeightCG(BusinessObject supportComponentBO)
        {
            ...
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, SmartPartLocalizer.GetString(SmartPartSymbolResourceIDs.ErrWeightCG, "Error in Weight CG of TypeE."));
                    return;
                }
            }
        }

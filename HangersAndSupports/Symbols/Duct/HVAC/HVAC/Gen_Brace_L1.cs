...
//   02/June/2013   Manikanth TR-CP-234520 Implemented TDL Issues
...
namespace Ingr.SP3D.Content.Support.Symbols
{
    ...
    [VariableOutputs]
    public class Gen_Brace_L1 : HangerComponentSymbolDefinition, ICustomHgrBOMDescription, ICustomWeightCG
    {
        //----------------------------------------------------------------------------------
        ...
        protected override void ConstructOutputs()
        {
            try
            {
                ...
                if (HgrCompareDoubleService.cmpdbl(width - flangeThickness, 0)==true)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, HVACLocalizer.GetString(HVACSymbolResourceIDs.ErrInvalidWidthFlangeThickness, "Width - Flange Thickness cannot be zero"));
                    return;
                }
                if (HgrCompareDoubleService.cmpdbl(depth, 0) == true)
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, HVACLocalizer.GetString(HVACSymbolResourceIDs.ErrInvalidDepth, "Depth cannot be zero"));
                    return;
                }
                //=================================================
                ...
                Z3 = (depth) / (Math.Tan((cutBackAngle1)));
                Z4 = (depth) / Math.Tan((cutBackAngle2));
                ...
                pointCollection.Add(new Position(L - Z4, depth, 0));
                pointCollection.Add(new Position(Z3, depth, 0));
                pointCollection.Add(new Position(0, 0, 0));
              
                Projection3d body1 = new Projection3d(new LineString3d(pointCollection), new Vector(0, 0, 1), flangeThickness, true);
                m_Symbolic.Outputs["BODY1"] = body1;

                pointCollection.Clear();

                pointCollection.Add(new Position(0, 0, flangeThickness));
                pointCollection.Add(new Position(L, 0, flangeThickness));
                ...
                Projection3d body2 = new Projection3d(new LineString3d(pointCollection), new Vector(0, 0, 1), width - flangeThickness, true);
            ...
            catch    //General Unhandled exception 
            {
                ...
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageError, HVACLocalizer.GetString(HVACSymbolResourceIDs.ErrConstructOutputs, "Error in ConstructOutputs of Gen_Brace_L1"));
                    return;
                }
            }
        }
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            catch    
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, HVACLocalizer.GetString(HVACSymbolResourceIDs.ErrConstructOutputs, "Error in BOMDescription of Gen_Brace_L1"));
                }
                return "";
            }
        }
        ...
        void ICustomWeightCG.EvaluateWeightCG(BusinessObject supportComponentBO)
        {
            try
            {
                ...
                double webThickness = (double)((PropertyValueDouble)catalogPart.GetPropertyValue("IJOAHgrHVACGenBrace", "WebT")).PropValue;
            ...
            catch
            {
                if (base.ToDoListMessage == null) //Check ToDoListMessgae created already or not
                {
                    ToDoListMessage = new ToDoListMessage(ToDoMessageTypes.ToDoMessageWarning, HVACLocalizer.GetString(HVACSymbolResourceIDs.ErrConstructOutputs, "Error in WeightCG of Gen_Brace_L1"));
                }
            }
        }
        #endregion

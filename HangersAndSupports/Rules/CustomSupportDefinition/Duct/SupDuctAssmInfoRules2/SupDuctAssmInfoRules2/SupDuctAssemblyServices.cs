    ...
    public static class SupDuctAssemblyServices
    {
        ...
        public static string GetPadPartNameByCrossSectionType(CustomSupportDefinition customSupportDefinition, PartInfo hgrBeam)
        {
            try
            {
                ...
                partClass = (PartClass)catalogBaseHelper.GetPartClass("HS_HgrServ_PadSelection");
                ...
                {
                    string section = ((string)((PropertyValueString)padItem.GetPropertyValue("IJUAHngServ_PadSelection", "SectionName")).PropValue);
                    if (section != null && section.ToUpper() == sectionName.ToUpper())
                    {
                ...
                if (sectionType == "L")
                    padPartClass = "S3Dhs_TriangularPad";
                else if (sectionType == "CS")
                    padPartClass = "S3Dhs_CircularPad";
                else if (sectionType == "W" || sectionName == "CS")
                {
                    ...
                    padPartClass = "S3Dhs_RectangularPad";
                }
                else
                    padPartClass = "S3Dhs_TriangularPad";
        ...
        /// <summary>
        /// This method returns the pad part part type by given pad name.
        /// </summary>
        /// <param name="customSupportDefinition">an abstract class, that contain the support definition.</param> 
        /// <param name="hgrBeam">Hanger beam PartInfo.</param> 
        /// <returns>String</returns>        
        /// <code>
        ///       parts.Add(new PartInfo(CustomSupportDefinition customSupportDefinition, string partNumber)
        ///</code>

        public static string GetPadPartType(CustomSupportDefinition customSupportDefinition, string partNumber)
        {
            try
            {
                CatalogBaseHelper catalogBaseHelper = new CatalogBaseHelper();
                ...
                foreach (BusinessObject padItem in classItems)
                {
                    if (((string)((PropertyValueString)padItem.GetPropertyValue("IJUAHngServ_PadSelection", "Part")).PropValue).ToUpper() == partNumber.ToUpper())
                    {
                        padType = (string)((PropertyValueString)padItem.GetPropertyValue("IJUAHngServ_PadSelection", "PadType")).PropValue;
                        break;
                    }
                }
                return padType;

...
//
//   Assy_FR_UC_LS.cs
//   PipeHgrAssemblies,Ingr.SP3D.Content.Support.Rules.Assy_FR_UC_LS
//   Author       :  Manikanth
...
//   -----------     ---     ------------------
//   20-03-2013     Manikanth   CR-CP-224484  Convert HS_Assembly to C# .Net  
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    public class Assy_FR_UC_LS : CustomSupportDefinition
    {
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                double angle, angle1, angle2, steelOffset = 0, byPointOffset = 0, byPointAngle1, byPointAngle2, structOffset = 0, offset = 0, offset1=0;
                ...
                Plane routePlane1 = new Plane(); Plane routePlane2 = new Plane(); Plane byPointPlane1 = new Plane(); Plane byPointPlane2 = new Plane();
                Axis routeAxis1 = new Axis(); Axis routeAxis2 = new Axis();Axis byPointAxis1 = new Axis(); Axis byPointAxis2 = new Axis();
                ...
                if (Math.Abs(byPointAngle2) < Math.PI / 2.0)       //The structure is oriented in the standard direction
                {
                    ...
                    {
                        byPointPlane1 = Plane.XY;
                        byPointPlane2 = Plane.XY;
                        byPointAxis1 = Axis.X;
                        byPointAxis2 = Axis.NegativeX;
                        byPointOffset = steelDepth / 2.0;
                    }
                    else
                    {
                        if (Configuration == 1)
                    ...
                    else
                    {
                        ...
                        if (SupportHelper.SupportingObjects.Count != 0)
                        {
                            if ((SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.Member || SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.HangerBeam))
                                JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.Y, Axis.NegativeY, plateThickness, pipeDiameter / 2 + steelDepth / 2, steelWidth / 2 + offset1);
                            else
                            {
                                if (HgrCompareDoubleService.cmpdbl(Math.Round(angle2, 3), Math.Round(Math.PI, 3)) == true)
                                    JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.Y, Axis.NegativeY, plateThickness, -offset1, -pipeDiameter / 2);
                                else
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if ((SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.Member || SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.HangerBeam))
                                JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.Y, Axis.NegativeY, 0.0, -pipeDiameter / 2, offset1);
                            else
                            {
                                if (HgrCompareDoubleService.cmpdbl(Math.Round(angle2, 3), Math.Round(Math.PI, 3)) == true)
                                    JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.Y, Axis.NegativeY, 0.0, -offset1, -pipeDiameter / 2);
                                else
                                    JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.X, Axis.X, 0.0, pipeDiameter / 2, 0);
                            }
                        }
                        else
                        {
                ...
                if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                    ...
                    JointHelper.CreateRigidJoint(HORCONNECTION, "Connection", TOP_SECTION, "BeginCap", byPointPlane1, byPointPlane2, byPointAxis1, byPointAxis2, 0.0, steelOffset, byPointOffset);
            ...
            {
                Type myType = this.GetType();
                CmnException e1 = new CmnException("Error in Get Assembly Joints Method of Assy_FR_UC_LS." + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name + ". Error:" + e.Message, e);
                throw e1;
            }
        }

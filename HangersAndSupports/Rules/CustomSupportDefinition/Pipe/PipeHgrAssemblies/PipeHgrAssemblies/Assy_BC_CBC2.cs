...
//
//   Assy_BC_CBC2.cs
//   PipeHgrAssemblies,Ingr.SP3D.Content.Support.Rules.Assy_BC_CBC2
//   Author       :  Vijay
...
//   -----------     ---     ------------------
//   16-04-2013     Vijay   CR-CP-224484  Convert HS_Assembly to C# .Net  
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    public class Assy_BC_CBC2 : CustomSupportDefinition
    {
            ...
            get
            {
                try
                {
                    ...
                    if (basePlate == "With")
                    {
                        ...
                        parts.Add(new PartInfo(ANGLE_SECTION, "Utility_CUTBACK_C2_1"));
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                if (Math.Abs(angle) < Math.Round(Math.PI, 2) / 2 && Math.Abs(angle) > 0)
                {
                    if (Configuration == 1)
                    {
                    ...
                    else if (Configuration == 3)
                    {
                        ...
                        if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                        {
                            ...
                            structOffset = steelWidth / 2;
                            offset = -(boundingBoxWidth / 2.0 + shoeH + steelDepth);
                        }
                        else
                        {
                            if ((SupportHelper.SupportingObjects.Count != 0))
                ...
                double tr1Leg1, tr1Leg2, tr1Leg3, z1, z4, plateT, orderLength, calc1, axisBraceOffset;
                string cutbackBOM;
                ...
                if (basePlate == "With")
                {
                    ...
                    plateT = (double)((PropertyValueDouble)topPlateConnectorPart.GetPropertyValue("IJUAHgrUtility_FOUR_HOLE_PLATE", "THICKNESS")).PropValue;
                    calc1 = steelDepth / Math.Cos(cutBackAngle);
                    tr1Leg1 = length + overHang - beamOverHang - plateT;
                    tr1Leg2 = tr1Leg1 / Math.Tan(cutBackAngle2);
                    tr1Leg3 = (length - plateT) / Math.Cos(cutBackAngle);
                    ...
                    componentDictionary[ANGLE_SECTION].SetPropertyValue(tr1Leg3, "IJOAHgrUtility_GENERIC_W", "L");
                    ...
                    componentDictionary[ANGLE_SECTION].SetPropertyValue(steelThickness, "IJOAHgrUtility_GENERIC_W", "T_FLANGE");
                    ...
                    if (Configuration == 1 || Configuration == 2)
                        JointHelper.CreateRigidJoint(TOP_PLATE, "TopStructure", BOT_PLATE, "TopStructure", Plane.XY, Plane.XY, Axis.X, Axis.X, 0, steelDepth / 2 + tr1Leg2, 0);
                ...
                {
                    plateT = 0;
                    tr1Leg1 = length + overHang - beamOverHang;
                    tr1Leg2 = tr1Leg1 / Math.Tan(cutBackAngle);
                ...
                if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                    JointHelper.CreateRigidJoint("-1", "Structure", TOP_SECTION, "BeginCap", confStructPlane[0], confStructPlane[1], confStructAxis[0], confStructAxis[1], plateT, structOffset, offset);
                else
                {
                    if ((SupportHelper.SupportingObjects.Count != 0))
                    {
                        if ((SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.Member || SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.HangerBeam) && HgrCompareDoubleService.cmpdbl(Math.Round(angle3, 3) , Math.Round(Math.Round(Math.PI, 2) / 2, 3))==true || HgrCompareDoubleService.cmpdbl(Math.Round(angle3, 3) , Math.Round(Math.Round(Math.PI, 2), 3))==true)
                            ...
                            JointHelper.CreateRigidJoint("-1", "Structure", TOP_SECTION, "BeginCap", confStructPlane[0], confStructPlane[1], confStructAxis[0], confStructAxis[1], plateT, offset, structOffset);
                    }
                    else
                        JointHelper.CreateRigidJoint("-1", "Structure", TOP_SECTION, "BeginCap", confStructPlane[0], confStructPlane[1], confStructAxis[0], confStructAxis[1], plateT, offset, structOffset);
                }
                ...
                if (Configuration == 1 || Configuration == 2)
                    ...
                    JointHelper.CreateRigidJoint(TOP_SECTION, "EndCap", ANGLE_SECTION, "EndStructure", Plane.ZX, Plane.YZ, Axis.X, Axis.NegativeY, 0, axisBraceOffset, originBraceOffset);

...
//
//   Assy_FR_UC_WS.cs
//   PipeHgrAssemblies,Ingr.SP3D.Content.Support.Rules.Assy_FR_UC_WS
//   Author       :  Manikanth
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    public class Assy_FR_UC_WS : CustomSupportDefinition
    {
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                double angle, angle1, angle2, steelOffset = 0, structOffset = 0, offset = 0, offset1=0;
                ...
                if (basePlate == "With")
                {
                    ...
                    else
                    {
                        ...
                        if (SupportHelper.SupportingObjects.Count != 0)
                        {
                            ...
                            {
                                if ((SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.Member || SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.HangerBeam))
                                    JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.Y, Axis.NegativeX, 0, steelDepth + pipeDiameter / 2, 0);
                                else
                                {
                                    if (HgrCompareDoubleService.cmpdbl(Math.Round(angle2, 3), Math.Round(Math.PI, 3)) == true)
                                        ...
                                        JointHelper.CreateRigidJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.X, Axis.X, plateThickness, steelDepth + pipeDiameter / 2 + offset1, 0);
                                }
                            }
                        }
                        else
                ...
                else
                {
                    //Add Connection for the end of the Angled Beam
                    if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                        JointHelper.CreatePrismaticJoint("-1", "Structure", HORCONNECTION, "Connection", Plane.XY, Plane.XY, Axis.X, Axis.X, 0, -structOffset);
                    else
                    {
            ...
            {
                Type myType = this.GetType();
                CmnException e1 = new CmnException("Error in Get Assembly Joints Method of Assy_FR_UC_WS." + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name + ". Error:" + e.Message, e);
                throw e1;
            }
        }
        ...
        public override Collection<ConnectionInfo> SupportingConnections
        {
            get
            {
                try
                {
                    ...
                    int clampBegin = 1;
                    for (int index = clampBegin; index <= numStruct; index++)
                    {
                        if (basePlate == "With")

...
//
//   USupport_2.cs
//   OglaendAssemblies,Ingr.SP3D.Content.Support.Rules.USupport_2
//   Author       :  PVK
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    //-----------------------------------------------------------------------------------
    public class USupport_2 : CustomSupportDefinition, ICustomHgrBOMDescription
    {
        ...
        public override Collection<PartInfo> Parts
        {
            get
            {
                try
                {
                    ...
                    isMember = OglaendAssemblyServices.AddPart(this, SECTION_2, member2Part, strRule, parts, ruleBO);
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                try
                {
                    InputHeight = (double)((PropertyValueDouble)support.GetPropertyValue("IJUAhsOGHeight2", "Height2")).PropValue;
                }
                catch
                {
                    InputHeight = 0;
                ...
                if ((HgrCompareDoubleService.cmpdbl(Math.Round(RSdistance2 * 1000, 0), Math.Round(RSDistance * 1000, 0)) == false))
                {
                    if (((Boolean)((PropertyValueBoolean)support.GetPropertyValue("IJUAhsLengthOverride", "LengthOverride")).PropValue) == true)
                    {
                        if (RSDistance > MaxHeight)
                        {
                            ...
                            support.SetPropertyValue(MaxHeight, "IJUAhsOGHeight2", "Height2");
                        }
                        else
                        {
                            ShoeHeight = 0;
                            support.SetPropertyValue(SupportHeight, "IJUAhsOGHeight2", "Height2");
                        }
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            support.SetPropertyValue(InputHeight, "IJUAhsOGHeight2", "Height2");
                        }
                    }
                }
                else
                ...
                try
                {
                    SectionHeight = (double)((PropertyValueDouble)support.GetPropertyValue("IJUAhsOGHeight1", "Height1")).PropValue;
                }
                catch
                {
                    SectionHeight = 0;
                ...
                if (HgrCompareDoubleService.cmpdbl(Math.Round(SectionHeight), 0) == true)
                {
                    SectionHeight = InputHeight / 2;
                    support.SetPropertyValue(SectionHeight, "IJUAhsOGHeight1", "Height1");
                }
                else
                    SectionHeight = (double)((PropertyValueDouble)support.GetPropertyValue("IJUAhsOGHeight1", "Height1")).PropValue;

                JointHelper.CreateAngularRigidJoint(SECTION_2, "Neutral", SECTION, "Neutral", new Vector(-0, SectionHeight - InputHeight, 0), new Vector(0, 0, 0));

...
//
//   SFS5380_AK2.cs
//   FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5380_AK2
//   Author       : Rajeswari
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    //-----------------------------------------------------------------------------------
    public class SFS5380_AK2 : CustomSupportDefinition
    {
        private const string PIPECLAMP1 = "PIPECLAMP1_AK2";
        private const string PIPECLAMP2 = "PIPECLAMP2_AK2";
        private const string TRIANGLEPLATE1 = "TRIANGLEPLATE1_AK2";
        private const string TRIANGLEPLATE2 = "TRIANGLEPLATE2_AK2";
        private const string SCREWLUG1 = "SCREWLUG1_AK2";
        private const string SCREWLUG2 = "SCREWLUG2_AK2";
        private const string STOPPER1 = "STOPPER1_AK2";
        private const string STOPPER2 = "STOPPER2_AK2";
        private const string LOGOBJ = "LOGOBJ_AK2";
        ...
        public override Collection<PartInfo> Parts
        {
            get
            {
                try
                {
                    ...
                    parts.Add(new PartInfo(TRIANGLEPLATE1 + Index, "FINLCmp_SFS5384"));
                    parts.Add(new PartInfo(TRIANGLEPLATE2 + Index, "FINLCmp_SFS5384"));
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                holeToHole = FINLAssemblyServices.GetDataByCondition("FINLCmp_SFS5384", "IJUAFINL_C", "C", "IJUAFINL_PipeND_mm", "PipeND", (nominalPipeDiameter - 0.001), (nominalPipeDiameter + 0.001));
                clampWidth = FINLAssemblyServices.GetDataByCondition("FINLCmp_SFS5370", "IJUAFINL_Width", "Width", "IJUAFINL_PipeND_mm", "PipeND", (nominalPipeDiameter - 0.001), (nominalPipeDiameter + 0.001));
                ...
                if (SupportHelper.SupportedObjects.Count > 1)
                {
                    ...
                    {
                        MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "Assembly may be too short.", "", "SFS5380_AK2.cs", 118);
                        return;
                    }
                }
                else
                {
                    ...
                    {
                        MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "Assembly may be too short.", "", "SFS5380_AK2.cs", 126);
                        return;
                    }
                }
                ...
                if (ndFrom > nominalDiameter || nominalDiameter > ndTo)
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARNINGMESSAGE", "ConfigureSupport" + ": " + "WARNING: " + "Load Class of Supported Pipe does not match Load Class of supporting Pipe.", "", "SFS5380_AK2.cs", 224);
                    ...
                    else
                    {
                        if (Math.Abs((routeXStructAngle) * 180 / Math.PI - 90) < 1)
                        {
                            if (Math.Abs((routeXStructAngle2) * 180 / Math.PI - 180) < 1)
                                JointHelper.CreateRigidJoint("-1", "Route", LOGOBJ + Index, "Connection", Plane.XY, Plane.ZX, Axis.X, Axis.NegativeY, 0, 0, 0);
                ...
                JointHelper.CreateRigidJoint(PIPECLAMP2 + Index, "Pin", TRIANGLEPLATE1 + Index, "Hole1", Plane.ZX, Plane.YZ, Axis.Z, Axis.NegativeZ, 0, 0, 0);
                ...
                if (support.SupportedObjects.Count == 2)
                    JointHelper.CreatePlanarJoint(PIPECLAMP1 + Index, "Route", "-1", "Route_2", Plane.YZ, Plane.ZX, 0);
                ...
                else
                {
                    if (supportingType != "Slab")
                    {
                        if(support.SupportingObjects.Count > 1)
                            JointHelper.CreatePlanarJoint(PIPECLAMP1 + Index, "Route", "-1", "Route", Plane.YZ, Plane.ZX, 0);
                    }
                }
            }
            catch (Exception e)

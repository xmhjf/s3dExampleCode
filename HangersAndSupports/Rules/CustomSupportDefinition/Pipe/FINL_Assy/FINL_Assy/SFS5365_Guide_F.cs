...
//
//   SFS5365_Guide_F.cs
//   FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5365_Guide_F
//   Author       :  BS
...
{
    public class SFS5365_Guide_F : CustomSupportDefinition, ICustomHgrBOMDescription
    {
        CustomSupportDefinition shoe, connObj1, connObj2, stopper, l1, l2;
        ...
        public override Collection<PartInfo> Parts
        {
            get
            {
                try
                {
                    ...
                    sectionSize = (string)((PropertyValueString)support.GetPropertyValue("IJUAFINLSectionSize", "SectionSize")).PropValue;
                    FINLAssemblyServices.GetCrossSectionDimensions("Euro", "L", sectionSize, out widthL, out thicknessL, out sectionL, out depthL);
                    ...
                    Collection<PartInfo> parts = new Collection<PartInfo>(shoe.Parts.Concat(l1.Parts).Concat(l2.Parts).ToList());
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                JointHelper.m_oCollOfJoints = new Collection<object>(shoeJoints.Concat(l1Joints).Concat(l2Joints).ToList());
                ...
                if (existSteelW > shoeLength)
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARNINGMESSAGE", "ConfigureSupport" + ": " + "WARNING: " + "Existing steel width should not exceed shoe length.", "", "SFS5365_Guide_F.cs", 194);
                ...
                if (npdMetric >= 50.00 / 1000.00 && (!slideType.Equals("SFS5379")) && clmaps == 1)
                {
                    ...
                    X = Math.Sin((varAngle - 90) / 180.0 * Math.PI) * hyp;
                    Y = Math.Cos((varAngle - 90) / 180.0 * Math.PI) * hyp;
            ...
            {
                Type myType = this.GetType();
                CmnException e1 = new CmnException("Error in Get Assembly Joints Method of SFS5365_Guide_F." + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name + ". Error:" + e.Message, e);
                throw e1;
            }
        }
        ...
        public string BOMDescription(BusinessObject SupportOrComponent)
        {
            ...
            {
                PipeObjectInfo pipeInfo = (PipeObjectInfo)SupportedHelper.SupportedObjectInfo(1);
                return BOMString = "Guide F SFS 5365 DN " + pipeInfo.NominalDiameter.Size;
            }
            catch (Exception e)
            {
                Type myType = this.GetType();
                CmnException e1 = new CmnException("Error in BOM of Assembly - SFS5365_Guide_F" + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name + ". Error:" + e.Message, e);
                throw e1;
            }
        }
        #endregion

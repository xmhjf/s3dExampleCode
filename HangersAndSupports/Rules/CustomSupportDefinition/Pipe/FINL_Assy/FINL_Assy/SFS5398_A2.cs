...
//
//   SFS5398_A2.cs
//   FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5398_A2
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    public class SFS5398_A2 : CustomSupportDefinition, ICustomHgrBOMDescription
    {
        int size4Value;
        string size4;
        double shoeH, X1, X2, steelOverlap, normalPipeDiameter;

        private const string HORSTEEL = "HorSteel_5398_A2";
        private const string VERSTEEL1 = "VerSteel1_5398_A2";
        private const string VERSTEEL2 = "VerSteel2_5398_A2";
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                {
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "Pipe size not valid.", "", "SFS5398_A2.cs", 115);
                    return;
                }

                //Getting Dimension Information
                //Interface name: IJUAFINLSrv_SFS5398_A2_Dim

                PropertyValue TSSize1, TSSize2;
                Dictionary<String, Object> parameter = new Dictionary<String, Object>();
                parameter.Add("Size4", size4);

                GenericHelper.GetDataByRule("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "TS_Size1", parameter, out TSSize1);
                GenericHelper.GetDataByRule("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "TS_Size2", parameter, out TSSize2);
                double maxSpan = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "Max_Span", "IJUAFINLSrv_SFS5398_A2_Dim", "Size4", size4);
                double maxLength = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "Max_Len", "IJUAFINLSrv_SFS5398_A2_Dim", "Size4", size4);
                double MA = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "MA", "IJUAFINLSrv_SFS5398_A2_Dim", "Size4", size4);
                double MP = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "MP", "IJUAFINLSrv_SFS5398_A2_Dim", "Size4", size4);
                double FMax = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5398_A2_Dim", "IJUAFINLSrv_SFS5398_A2_Dim", "Fmax", "IJUAFINLSrv_SFS5398_A2_Dim", "Size4", size4);
                ...
                double horizontalLength, verticalLength, existSteelD=0;
                ...
                {
                    if (SupportHelper.PlacementType != PlacementType.PlaceByReference && SupportingHelper.SupportingObjectInfo(1).SupportingObjectType == SupportingObjectType.Member)
                ...
                if (verticalDistance + shoeH + pipeInfo.OutsideDiameter / 2 + depthHorTS > maxLength)
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARNINGMESSAGE", "ConfigureSupport" + ": " + "WANRING: " + "Maximum allowable length of " + Convert.ToString(maxLength * 1000) + "mm exceeded.", "", "SFS5398_A2.cs", 166);

                //If the span length entered is greater than allowed
                if (X1 + X2 > maxSpan)
                {
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "Maximum allowable span of " + Convert.ToString(maxLength * 1000) + "mm exceeded.", "", "SFS5398_A2.cs", 171);
                    return;
                }

                if (steelOverlap < (100.0 / 1000.0))
                {
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "Minimum Overlap is " + Convert.ToString(100) + " mm", "", "SFS5398_A2.cs", 177);
                    return;
                }
                if (pipeInfo.OutsideDiameter / 2 > X1 || pipeInfo.OutsideDiameter / 2 > X2)
                {
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "X1 and X2 must exceed Pipe outside radius.", "", "SFS5398_A2.cs", 182);
                    return;
                }
                ...
                if (supportingType == "Steel")
                {
                    if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                        ...
                        MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARNINGMESSAGE", "ConfigureSupport" + ": " + "WANRING: " + "Support can only be placed on steel By-Structure.", "", "SFS5398_A2.cs", 218);
                }
                else
                {
                    if (SupportHelper.PlacementType == PlacementType.PlaceByReference || SupportHelper.PlacementType == PlacementType.PlaceByPoint)
                    {
                        ...
                        MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARNINGMESSAGE", "ConfigureSupport" + ": " + "WANRING: " + "Support can only be placed on Slab By-Point.", "", "SFS5398_A2.cs", 230);
                }
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                BOMString = "Gate support A2 -" + Convert.ToString(sizeBOM) + " SFS 5398";

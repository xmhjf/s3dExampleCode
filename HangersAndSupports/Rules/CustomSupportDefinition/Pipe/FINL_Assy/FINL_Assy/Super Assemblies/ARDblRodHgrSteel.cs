...
//
//   ARDblRodHgrSteel.cs
//   FINL_Assy,Ingr.SP3D.Content.Support.Rules.ARDblRodHgrSteel
//   Author       :  Hema
...
{
    public class ARDblRodHgrSteel : CustomSupportDefinition, ICustomHgrBOMDescription
    {
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                {
                    if (subTopAssembly1.GetType().FullName.Equals("Ingr.SP3D.Content.Support.Rules.SFS5380_YK3") && (SupportHelper.PlacementType == PlacementType.PlaceByPoint || SupportHelper.PlacementType == PlacementType.PlaceByReference))
                    {
                        JointHelper.CreateRevoluteJoint(subAssyRouteConnections1[0].PartKey, "BotExThdRH", subAssyStructConnections[0].PartKey, "Hole2", Axis.Z, Axis.Z);
                        JointHelper.CreateRevoluteJoint(subAssyRouteConnections2[0].PartKey, "BotExThdRH", subAssyStructConnections[0].PartKey, "Hole1", Axis.Z, Axis.Z);
                    }
                    else
                    {
                ...
                {
                    JointHelper.CreateSphericalJoint(subAssyRouteConnections1[0].PartKey, "BotExThdRH", subAssyStructConnections[1].PartKey, "InThdRH");
                    JointHelper.CreateSphericalJoint(subAssyRouteConnections2[0].PartKey, "BotExThdRH", subAssyStructConnections[0].PartKey, "InThdRH");
                }
        ...
        public string BOMDescription(BusinessObject SupportOrComponent)
        {
            ...
            try
            {
                ...
                double d5384_CE = 0, E = 0,length;

                double bottomPipeNominalDiameter = MiddleServiceProvider.UOMMgr.ConvertUnitToUnit(UnitType.Npd, pipeInfo.NominalDiameter.Size, MiddleServiceProvider.UOMMgr.GetUnit(UnitType.Npd, pipeInfo.NominalDiameter.Units), UnitName.NPD_MILLIMETER) / 1000;
                if (subBottom.Equals("FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5380_AR4"))
                    length = RefPortHelper.DistanceBetweenPorts("Route", "Structure", PortDistanceType.Vertical) + bottomPipeNominalDiameter / 2 + shoeHeight; //+ HH.GetAttr("ShoeH")
                else
                {
                    d5384_CE = FINLAssemblyServices.GetDataByCondition("FINLCmp_SFS5384", "IJUAFINL_C", "C", "IJUAFINL_PipeND_mm", "PipeND", bottomPipeNominalDiameter);
                    E = FINLAssemblyServices.GetDataByCondition("FINLCmp_SFS5384", "IJUAFINL_E", "E", "IJUAFINL_PipeND_mm", "PipeND", bottomPipeNominalDiameter);
                    d5384_CE = d5384_CE + E;
                ...
                if (subTop1.Equals("FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5380_YK1"))
                    top = "YK1";
                else if (subTop1.Equals("FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5380_YK2"))
                    top = "YK2";

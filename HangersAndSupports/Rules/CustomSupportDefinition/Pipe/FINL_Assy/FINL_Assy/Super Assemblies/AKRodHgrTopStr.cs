...
//
//   AKRodHgrTopStr.cs
//   FINL_Assy,Ingr.SP3D.Content.Support.Rules.AKRodHgrTopStr
//   Author       :  BS
...
{
    public class AKRodHgrTopStr : CustomSupportDefinition, ICustomHgrBOMDescription
    {
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                if (subBottomAssembly.GetType().FullName.Equals("Ingr.SP3D.Content.Support.Rules.SFS5380_AK6"))
                    JointHelper.CreateRevoluteJoint(subAssyRouteConnections[0].PartKey, "BotExThdRH", subAssyStructConnections[0].PartKey, "InThdRH", Axis.Z, Axis.NegativeZ);
                else if (subBottomAssembly.GetType().FullName.Equals("Ingr.SP3D.Content.Support.Rules.SFS5380_AK1") && (subTopAssembly.GetType().FullName.Equals("Ingr.SP3D.Content.Support.Rules.SFS5380_YK4")))
                    JointHelper.CreateRevoluteJoint(subAssyRouteConnections[0].PartKey, "BotExThdRH", subAssyStructConnections[0].PartKey, "InThdRH", Axis.Z, Axis.NegativeZ);
                else
                    JointHelper.CreateRevoluteJoint(subAssyRouteConnections[0].PartKey, "BotExThdRH", subAssyStructConnections[0].PartKey, "InThdRH", Axis.Z, Axis.Z);
                ...
                else if (subBottomAssemblyName.Equals("Ingr.SP3D.Content.Support.Rules.SFS5380_AK3"))
                {
                    if (maxTemperature <= 20)
            ...
            {
                Type myType = this.GetType();
                CmnException e1 = new CmnException("Error in Get Assembly Joints Method of AKRodHgrTopStr." + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name + ". Error:" + e.Message, e);
                throw e1;
            }
        }
        ...
        public string BOMDescription(BusinessObject SupportOrComponent)
        {
            ...
            try
            {
                ...
                if (subTop.Equals("FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5380_YK4"))
                    top = "YK4";
            ...
            {
                Type myType = this.GetType();
                CmnException e1 = new CmnException("Error in BOM of Assembly - AKRodHgrTopStr" + myType.Assembly.FullName + "," + myType.Namespace + "." + myType.Name + ". Error:" + e.Message, e);
                throw e1;
            }
        }
        #endregion

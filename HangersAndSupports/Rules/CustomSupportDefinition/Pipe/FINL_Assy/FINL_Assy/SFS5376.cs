...
//
//   SFS5376.cs
//   FINL_Assy,Ingr.SP3D.Content.Support.Rules.SFS5376
//   Author       :  Vijay
...
namespace Ingr.SP3D.Content.Support.Rules
{
    ...
    public class SFS5376 : CustomSupportDefinition, ICustomHgrBOMDescription
    {
        double rotation, normalPipeDiameterMetric;
        string material;

        private const string VERPLATE = "VerPlate_5376";
        private const string HORPLATE = "HorPlate_5376";
        private const string CONNOBJECT = "ConnObj_5376";
        private const string ROUTECONNOBJECT = "RouteConnObject_5376";
        private const string PIPECLAMP1 = "PipeClamp1_5376";
        private const string PIPECLAMP2 = "PipeClamp2_5376";
        ...
        public override Collection<PartInfo> Parts
        {
            get
            {
                try
                {
                    ...
                    {
                        MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "Parts" + ": " + "ERROR: " + "Pipe size not valid.", "", "SFS5376.cs", 92);
                        return parts;
                    }
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                //Interface name: IJUAFINLSrv_SFS5376_Dim:: Pipe_Nom_Dia_m, Shoe_H Shoe_W, B ,T ,L ,Weight , Max_Load
                double shoeH = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5376_Dim", "IJUAFINLSrv_SFS5376_Dim", "Shoe_H", "IJUAFINLSrv_SFS5376_Dim", "Pipe_Nom_Dia_m", Convert.ToString(normalPipeDiameterMetric * 1000));
                double B = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5376_Dim", "IJUAFINLSrv_SFS5376_Dim", "B", "IJUAFINLSrv_SFS5376_Dim", "Pipe_Nom_Dia_m", Convert.ToString(normalPipeDiameterMetric * 1000));
                double T = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5376_Dim", "IJUAFINLSrv_SFS5376_Dim", "T", "IJUAFINLSrv_SFS5376_Dim", "Pipe_Nom_Dia_m", Convert.ToString(normalPipeDiameterMetric * 1000));
                double length = FINLAssemblyServices.GetDataByCondition("FINLSrv_SFS5376_Dim", "IJUAFINLSrv_SFS5376_Dim", "L", "IJUAFINLSrv_SFS5376_Dim", "Pipe_Nom_Dia_m", Convert.ToString(normalPipeDiameterMetric * 1000));
                ...
                plateThickness = T;
                string verticalPlateBom = "Plate SFS 2022 " + MiddleServiceProvider.UOMMgr.FormatUnit(UnitType.Distance, shoeH - clampThickness - plateThickness, UnitName.DISTANCE_MILLIMETER).Split('.').GetValue(0) + "x" + MiddleServiceProvider.UOMMgr.FormatUnit(UnitType.Distance, length, UnitName.DISTANCE_MILLIMETER).Split('.').GetValue(0) + "x" + MiddleServiceProvider.UOMMgr.FormatUnit(UnitType.Distance, plateThickness, UnitName.DISTANCE_MILLIMETER).Split('.').GetValue(0);
                componentDictionary[VERPLATE + "_" + Index].SetPropertyValue(shoeH - clampThickness - plateThickness, "IJOAHgrUtility_USER_FIXED_BOX", "L");
                componentDictionary[VERPLATE + "_" + Index].SetPropertyValue(plateThickness, "IJOAHgrUtility_USER_FIXED_BOX", "WIDTH");
                componentDictionary[VERPLATE + "_" + Index].SetPropertyValue(length, "IJOAHgrUtility_USER_FIXED_BOX", "DEPTH");
                componentDictionary[VERPLATE + "_" + Index].SetPropertyValue(verticalPlateBom, "IJOAHgrUtility_USER_FIXED_BOX", "BOM_DESC");

                string horizontalPlateBom = "Plate SFS 2022 " + MiddleServiceProvider.UOMMgr.FormatUnit(UnitType.Distance, B, UnitName.DISTANCE_MILLIMETER).Split('.').GetValue(0) + "x" + MiddleServiceProvider.UOMMgr.FormatUnit(UnitType.Distance, length, UnitName.DISTANCE_MILLIMETER).Split('.').GetValue(0) + "x" + MiddleServiceProvider.UOMMgr.FormatUnit(UnitType.Distance, plateThickness, UnitName.DISTANCE_MILLIMETER).Split('.').GetValue(0);
                ...
                componentDictionary[HORPLATE + "_" + Index].SetPropertyValue(length, "IJOAHgrUtility_USER_FIXED_BOX", "DEPTH");
                ...
                if (Clamps == 1)      //with one or two clamps
                {
                    ...
                    if ((Math.Abs(verticalAngle) > 40 + 0.0001) && Symbols.HgrCompareDoubleService.cmpdbl(Math.Round(Math.Abs(verticalAngle)), 45) == false)
                        MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARNINGMESSAGE", "ConfigureSupport" + ": " + "WARNING: " + "Clamp rotation cannot exceed 40 degrees.", "", "SFS5376.cs", 188);
        ...
        public string BOMDescription(BusinessObject oSupportOrComponent)
        {
            ...
            try
            {
                ...
                if (Symbols.HgrCompareDoubleService.cmpdbl(rotationbom, 0) == true)
                    BOMString = "Pipe slide SFS 5376 DN " + Convert.ToString(pipeDiameter);
                else
                    BOMString = "Pipe slide SFS 5376 DN " + Convert.ToString(pipeDiameter) + " -" + Convert.ToString(Math.Round(angle, 0)) + Convert.ToString(176);

...
//
//   PU1.cs
//   FLSampleSupports,Ingr.SP3D.Content.Support.Rules.PU1
//   Author       : Rajeswari
...
namespace Ingr.SP3D.Content.Support.Rules
{
   ...
   public class PU1 :CustomSupportDefinition , ICustomHgrBOMDescription
    {
       ...
       string sectionSize;
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                {
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERERRORMESSAGE", "ConfigureSupport" + ": " + "ERROR: " + "Can not place-by-structure please place-by-point.", "", "PU1.cs", 83);
                    return;
                }
                ...
                if(routeInfo.InsulationThickness > 0)
                    MiddleServiceProvider.ErrorLogger.Log(0, "", "", "USERWARININGMESSAGE", "ConfigureSupport" + ": " + "WARINING: " + "Not placable for Insulated pipe.", "", "PU1.cs", 118);
               ...
               componentDictionary[PLATE].SetPropertyValue(plateW, "IJOAHgrUtility_VARIABLE_BOX", "WIDTH");
               componentDictionary[PLATE].SetPropertyValue(plateH, "IJOAHgrUtility_VARIABLE_BOX", "DEPTH");
               componentDictionary[PLATE].SetPropertyValue(plateT, "IJUAHgrOccLength", "Length");
               componentDictionary[PLATE].SetPropertyValue("50x50x6 THK Carbon Steel Plate.", "IJOAHgrUtility_VARIABLE_BOX", "BOM_DESC");
                ...
                if (Configuration == 1)
                {
                    ...
                    JointHelper.CreateRigidJoint("-1", "BBRV_Low", HANGERBEAM, "EndCap", Plane.XY, Plane.NegativeZX, Axis.X, Axis.NegativeX, -shoeHeight, -hangerOffsetRight, 0.045);
                }
                else if (Configuration == 2)
                {
                    JointHelper.CreateRigidJoint(HANGERBEAM, "BeginCap", PLATE, "EndOther", Plane.XY, Plane.XY, Axis.X, Axis.X, 0, 0, hangerW / 2);
                    JointHelper.CreateRigidJoint("-1", "BBRV_Low", HANGERBEAM, "EndCap", Plane.XY, Plane.NegativeZX, Axis.X, Axis.X, -shoeHeight, boundingBoxWidth + hangerOffsetRight, -0.045);
                }
                else if (Configuration == 3)
                {
                    JointHelper.CreateRigidJoint(HANGERBEAM, "EndCap", PLATE, "StartOther", Plane.XY, Plane.XY, Axis.X, Axis.X, 0, 0, hangerW / 2);
                    JointHelper.CreateRigidJoint("-1", "BBRV_Low", HANGERBEAM, "BeginCap", Plane.XY, Plane.NegativeZX, Axis.X, Axis.NegativeX, -shoeHeight, boundingBoxWidth + hangerOffsetRight, 0.045);
                }
                else if (Configuration == 4)
                {
                    JointHelper.CreateRigidJoint(HANGERBEAM, "EndCap", PLATE, "StartOther", Plane.XY, Plane.XY, Axis.X, Axis.X, 0, 0, hangerW / 2);
                    JointHelper.CreateRigidJoint("-1", "BBRV_Low", HANGERBEAM, "BeginCap", Plane.XY, Plane.NegativeZX, Axis.X, Axis.X, -shoeHeight, -hangerOffsetRight, -0.045);
                }
            }
            catch (Exception e)
            {
        ...
        {
            return "PU1 - Pick-Up from Single non-Insulated Line.";
        }
        #endregion
        /// <summary>

...
//
//   G4G_1400_CTA.cs
//   CabletrayAssy2,Ingr.SP3D.Content.Support.Rules.ClipHoldClampG4G_1400_CTA
//   Author       : Vinay
    ...
    public class G4G_1400_CTA : CustomSupportDefinition
    {
        ...
        public override Collection<PartInfo> Parts
        {
            get
            {
                try
                {
                    ...
                    partClass[2] = partClass[3] = partClass[1] = partClass[4] = "RichHgrAISC31_L";

                    for (int i = 5; i <= noOfParts; i++)
                    {
                        partClass[i] = "S3Dhs_SmartPartPlate_3";
                    }
                    ...
                    for (int i = 1; i <= partClass.Length - 1; i++)
                    {
                        ...
                        if (partClass[i] == "S3Dhs_SmartPartPlate_3")
                        {
                            parts.Add(new PartInfo(part[i], "S3Dhs_HSA_SH_1"));
                        }
                        else if (partClass[i] == "RichHgrAISC31_L")
                        {
                            FlatPlate = supportComponentUtils.GetPartFromPartClass("RichHgrAISC31_L", "HgrCrossSectionByCW", support);
        ...
        public override void ConfigureSupport(Collection<SupportComponent> oSupCompColl)
        {
            try
            {
                ...
                double ctLength = 1.1 * dWidth;
                double ctWidth = 4 * dWidth;
                double thickness = 0.1; //(4 in)'provide const thickness for CT '0.5 * dHeight

                for (int i = 5; i <= noOfParts; i++)
                {
                    (componentDictionary[part[i]]).SetPropertyValue(ctWidth, "IJUAhsWidth1", "Width1");
                    (componentDictionary[part[i]]).SetPropertyValue(ctLength, "IJUAhsLength1", "Length1");
                    (componentDictionary[part[i]]).SetPropertyValue(thickness, "IJUAhsThickness1", "Thickness1");
                }
                 //---------------------------------------
                 // Set offset value
                 //---------------------------------------
                ...
                if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                    if (bbxLow == -1)
                        JointHelper.CreatePrismaticJoint("-1", strBBXLow, part[5], "Port1", Plane.XY, Plane.XY, Axis.X, Axis.X, -thickness - trayOffset, dWidth / 2);
                    else
                        JointHelper.CreatePrismaticJoint(part[bbxLow], strBBXLow, part[5], "Port1", Plane.XY, Plane.XY, Axis.X, Axis.X, -thickness - trayOffset, dWidth / 2);
                else
                    if (bbxLow == -1)
                        JointHelper.CreateRigidJoint("-1", strBBXLow, part[5], "Port1", Plane.XY, Plane.XY, Axis.X, Axis.X, -thickness - trayOffset, dWidth / 2, dPosOffset);
                    else
                        JointHelper.CreateRigidJoint(part[bbxLow], strBBXLow, part[5], "Port1", Plane.XY, Plane.XY, Axis.X, Axis.X, -thickness - trayOffset, dWidth / 2, dPosOffset);


                JointHelper.CreateRigidJoint(part[5], "Port1", part[1], "BeginCap", Plane.XY, Plane.XY, Axis.X, Axis.NegativeY, 0, ctLength/2, ctWidth / 2);
                JointHelper.CreateRigidJoint(part[5], "Port1", part[2], "BeginCap", Plane.XY, Plane.XY, Axis.X, Axis.X, 0, ctLength/2, -ctWidth / 2);
                JointHelper.CreateRigidJoint(part[5], "Port1", part[3], "EndCap", Plane.XY, Plane.NegativeXY, Axis.X, Axis.NegativeY, 0, -ctLength / 2, ctWidth / 2);
                JointHelper.CreateRigidJoint(part[5], "Port1", part[4], "EndCap", Plane.XY, Plane.NegativeXY, Axis.X, Axis.X, 0, -ctLength / 2, -ctWidth / 2);
                ...
                if (SupportHelper.PlacementType == PlacementType.PlaceByStruct)
                    JointHelper.CreatePointOnPlaneJoint(part[5], "Port1", structure.ToString(), strStructure, Plane.ZX);
                for (int i = 6; i <= noOfParts; i++)
                {
                    JointHelper.CreateRigidJoint(part[5], "Port1", part[i], "Port1", Plane.XY, Plane.XY, Axis.X, Axis.X, tierSpacing * (i - 5), 0, 0);
                }
        ...
        public override Collection<ConnectionInfo> SupportingConnections
        {
            get
            {
                try
                {
                    ...
                    structConnections.Add(new ConnectionInfo(part[1], leg1StructIdx)); // partindex, structureindex
                    structConnections.Add(new ConnectionInfo(part[2], leg2StructIdx)); // partindex, structureindex
                    structConnections.Add(new ConnectionInfo(part[3], leg3StructIdx)); // partindex, structureindex
                    structConnections.Add(new ConnectionInfo(part[4], leg4StructIdx)); // partindex, structureindex

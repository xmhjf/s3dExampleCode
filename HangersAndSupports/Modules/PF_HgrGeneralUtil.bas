...
'   Oct 05, 2010     AS              TR-CP-186615 - Handled Long Data type in SetupInputsAndOutputs and SetupInputsAndOutputsForCache
'   Oct 25, 2010     AS              TR-CP-187406- Modified the tInputs datatype to Hold bOptionalInput(), Modified AddInput Function to handle Optional Inputs
'   Nov 01, 2012     PR              DM 223373  Application error when defining workspace in specific DB
'+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
Public Function SetupInputsAndOutputsForCache(pSymbolDefinition As IMSSymbolEntities.IJDSymbolDefinition, _
        ByVal NUM_INPUTS As Integer, ByVal NUM_OUTPUTS As Integer, InputName() As String, _
        DataType() As String, OutputName() As String, m_progID As String, Optional IsOptionalInput As Boolean = False)
Const METHOD = "SetupInputsAndOutputsForCache"
    ...
    For N = 1 To NUM_INPUTS
        ...
        If IsOptionalInput = True Then
            SymbolInput(N).Properties = igINPUT_IS_A_PARAMETER Or igDESCRIPTION_OPTIONAL
        Else
            SymbolInput(N).Properties = igINPUT_IS_A_PARAMETER
        End If
...
Const METHOD = "LogError"
    ...
    'On Error Resume Next  ' This call clears all the VB Error collection
    ...
    On Error GoTo ErrHandler ' This call clears all the VB Error collection
...
'         - IsOptionalInput; Check the attribute is Optional
...
'
' ---------------------------------------------------------------------------
Public Sub AddInput(Inputs As tInputs, strName As String, sTrType As String, Optional IsOptionalInput As Boolean = False)
Const METHOD = "AddInput"
    On Error GoTo ErrorHandler
    
    Dim iSize As Long
    Static iIdx As Long
    Dim bOptionalInput As Boolean
    
    bOptionalInput = IsOptionalInput
    ...
    If iSize = -1 Or iSize < iIdx Then
        ...
        ReDim Preserve Inputs.bOptionalInput(iSize + 50)
    End If
    ...
    If bOptionalInput = True Then
        Inputs.bOptionalInput(iIdx) = True
    Else
        Inputs.bOptionalInput(iIdx) = False
    End If

...
'
'  Class    : ConduitCompSeqGen
'
'  Abstract : The file contains implementation of the naming rules for ConduitComponents with sequence ID's.
'
'***************************************************************************************
...
Public Class ConduitCompSeqGen
    ...
    Public Overrides Sub ComputeName(ByVal oEntity As Common.Middle.BusinessObject, ByVal oParents As System.Collections.ObjectModel.ReadOnlyCollection(Of Common.Middle.BusinessObject), ByVal oActiveEntity As Common.Middle.BusinessObject)
        Dim strCompName As String, strRunName As String, strSequenceNo As String
        ...
        Dim sMethod = "ConduitCompSeqGen : ComputeName() "
        If oEntity Is Nothing Or oParents Is Nothing Then
            Exit Sub
        End If
        Try
            'To get the Syatem Parent name
            oParent = New NamedItemHelper(oParents.Item(0))
            strRunName = oParent.Name

            'To get sequence number and append it
            Dim SeqID As PropertyValueString
            SeqID = oEntity.GetPropertyValue("IJSequence", "Id")
            strSequenceNo = SeqID.PropValue
            strCompName = strRunName + "-" + "COND" + "-" + strSequenceNo

            oNamedItem = New NamedItemHelper(oEntity)
            If (oNamedItem.Name <> strCompName) Then

...
'
'  Class    : DuctCompNameRule
'
'  Abstract : The file contains implementation of the naming rules for DuctComponents.
'
'***************************************************************************************
...
Public Class DuctCompNameRule
    ...
    Public Overrides Sub ComputeName(ByVal oEntity As Common.Middle.BusinessObject, ByVal oParents As System.Collections.ObjectModel.ReadOnlyCollection(Of Common.Middle.BusinessObject), ByVal oActiveEntity As Common.Middle.BusinessObject)
        ...
        Dim strRunName As String, strCompName As String, strSequenceNo As String, strLocation As String, strTypeString As String
        Dim lcount As Long
        Dim oLogError As LogError
        oLogError = MiddleServiceProvider.ErrorLogger()
        Dim sMethod = "DuctCompNameRule : ComputeName() "
        ...
        Try
            ...
            If strRunName <> GetNamingParentsString(oActiveEntity) Then                
                ...
                strTypeString = GetTypeString(oEntity)

                'Name is Formed in the Required Sequence
                strCompName = strRunName + "-" + strTypeString + "-" + strSequenceNo

...
{
    public class LoadCombinationDescRule : NameRuleBase
    {
        ...
        public override void ComputeName(BusinessObject entity, ReadOnlyCollection<BusinessObject> parents, BusinessObject activeEntity)
        {
            try
            {
                ...
                long count;
                BusinessObject spacePart;

                RelationCollection relationHelper = entity.GetRelationship("SpatialCombinationCatalog", "CombinationData");

                if (relationHelper.TargetObjects.Count == 1)
                {
                    spacePart = relationHelper.TargetObjects[0];
                }
                else
                {
                   return;
                ...
                if (spacePart != null)
                {
                    DataItem part = (DataItem)spacePart;

...
'
'Abstract:
'   Bas file with functions to support CreateBasePlane
'
'Description:
'History :
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Public Function CreateAvgOfCornersPlane(ByVal oPlatePart As IJPlatePart, ByVal strDirection As String, ByVal oSurfaceBody As IJSurfaceBody, ByVal bBaseSide As Boolean) As Plane3d
Const METHOD = "CreateAvgOfCornersPlane"
    ...
    Set oPlateEdges = oGeomHelper.GetPlatePartEdgesInIJElements(oSurfaceBody, bBaseSide)
    On Error GoTo ErrorHandler
    ...
    If oPlateEdges.count < 3 Then 'this is legal, use natural balance
        ...
        Set CreateAvgOfCornersPlane = GetTrueNaturalBalance(oPlatePart)
    ...
    Else 'four or more edges
        ...
        Set oTempEdges = GetEdgesAtGivenDirectionAndPlateNew3(oSurfaceBody, oPlateEdges, strDirection)
        On Error GoTo ErrorHandler
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Public Function CreateBPlaneOfPerpendicularXY(ByVal oProcessSettings As IJMfgTemplateProcessSettings, ByVal oPlatePart As IJPlatePart, ByVal oSurfaceBody As IJSurfaceBody, ByVal bBaseSide As Boolean) As IJPlane
...
End Function
'checks whether given two points are same
Public Function GetTrueNaturalBalance(ByVal oPlatePart As Object) As IJPlane
Const METHOD = "GetTrueNaturalBalance"
On Error GoTo ErrorHandler

    Dim oIJPlane As IJPlane
    Dim oPlaneUtil As New PinJigFloorUtils
    Dim oPlatePartElems As IJElements
    Set oPlatePartElems = New JObjectCollection
    oPlatePartElems.Add oPlatePart
   'calculate base plane
    Set oIJPlane = oPlaneUtil.TrueNaturalBalanceMethod(oPlatePartElems, 1) '1 meter baseplane height will be adjusted later
    
    Dim dRootX As Double, dRootY As Double, dRootZ As Double
    Dim dNormalX As Double, dNormalY As Double, dNormalZ As Double
        
    SnapPlaneToCS oIJPlane, 5#
    
    oIJPlane.GetRootPoint dRootX, dRootY, dRootZ
    oIJPlane.GetNormal dNormalX, dNormalY, dNormalZ
    
    Dim oBasePlane As IJPlane
    Set oBasePlane = New Plane3d
    
    oBasePlane.DefineByPointNormal dRootX, dRootY, dRootZ, dNormalX, dNormalY, dNormalZ
...
Public Function CreateParallelAxisPlane(ByVal oPlatePart As IJPlatePart, ByVal oSurfaceBody As IJSurfaceBody) As Plane3d
...
Public Function GetBasePlaneFromCorners(ByVal oPlatePart As Object, strBasePlane As String, ByVal oSurfaceBody As IJSurfaceBody, ByVal bBaseSide As Boolean) As IJPlane
...
Public Function GetPortSurface(oPlatePart As Object, ByVal strTemplateSide As String) As IJSurfaceBody
   ...
   If TypeOf oPlatePart Is IJConnectable Then
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' CreateBPlaneBasedOnMidTemplate
'   1. Find the mid template bottom line and related chord line
'   2. Compute the normal to Mid template BL chord line
'   3. Construct Base plane using bow string normal
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Public Function CreateBPlaneBasedOnMidTemplate(ByVal oTemplSetData As TemplSetData, ByVal oTemplateSet As Object) As IJPlane
    Const METHOD = "CreateBPlaneBasedOnMidTemplate"
    On Error GoTo ErrorHandler
    
    Dim oGeomHelper As MfgGeomHelper
    Set oGeomHelper = New MfgGeomHelper
    
    ' 1. Get the the Template Bottom curve
    Dim oMidTemplateWB  As IJWireBody
    Set oMidTemplateWB = GetMidTemplateBottomCurve(oTemplateSet, oTemplSetData)
    
    Dim oStartPos As IJDPosition, oEndPos As IJDPosition
    oMidTemplateWB.GetEndPoints oStartPos, oEndPos
    
    ' 2. Create Template bottom curve chord line
    Dim oLine As IJLine
    ...
    ' 3. Find the middle point of template bottom curve
    Dim oBCMidPos   As IJDPosition
    Set oBCMidPos = oMfgRuleHelpers.GetMiddlePoint(oMidTemplateWB)
    
    ' 4. Get nearest point on chord line from bottom curve mid pos
    Dim oChordPos   As IJDPosition
    oGeomHelper.NearestPtOnCurveFromPoint Nothing, oBCMidPos, oLine, oChordPos
    
    ' 5. Get the vector(will be used for BCl proj and BP) from proj pos and BC mid pos
    Dim oPlaneNormalVec As DVector
    Set oPlaneNormalVec = oBCMidPos.Subtract(oChordPos)
    
    oPlaneNormalVec.Length = 1

    Dim oBasePlane As IJPlane
    Set oBasePlane = New Plane3d
    
    oBasePlane.DefineByPointNormal oChordPos.x, oChordPos.y, oChordPos.z, oPlaneNormalVec.x, oPlaneNormalVec.y, oPlaneNormalVec.z
    
    Set CreateBPlaneBasedOnMidTemplate = oBasePlane
    Exit Function
...
Private Function GetMidTemplateBottomCurve(oTemplateSet As IJDMfgTemplateSet, oTemplSetData As TemplSetData) As Object
    Const METHOD = "GetMidTemplateBottomCurve"
    On Error GoTo ErrorHandler
    ...
    If oTemplateSet Is Nothing Then
        ...
        GoTo ErrorHandler
    End If
    If oTemplSetData Is Nothing Then
        strErrorMsg = strErrorMsg & "oTemplSetData is Nothing "
        GoTo ErrorHandler
    End If
    ...
    Dim nIndex As Long, lFrameCount As Long, lMidTemplateFrame As Long
    Dim oFrame As Object
    Dim oFrameElems As IJElements
 
    Dim oSurfaceBody As IJSurfaceBody
    Set oSurfaceBody = oTemplSetData.SurfaceBody
     
    ' make PositionFrames higher priority than PositionEven.
    Set oFrameElems = oTemplSetData.FrameElems
    
    If Not oFrameElems Is Nothing Then
        lFrameCount = oFrameElems.count
        If lFrameCount > 0 Then
            lMidTemplateFrame = (lFrameCount + 1) / 2
            Set oFrame = oFrameElems.Item(lMidTemplateFrame)
            
            Dim oIntersectCurve     As Object
            Dim oStartPos As IJDPosition, oEndPos As IJDPosition
            
            On Error GoTo ISWP_ErrorHandler 'ISWP-->IntersectSurfaceWithPlane(this does not retun if there is no intersection)
            oMfgGeomHelper.IntersectSurfaceWithPlane oSurfaceBody, oFrame, oIntersectCurve, oStartPos, oEndPos
            On Error GoTo ErrorHandler
            
            Set GetMidTemplateBottomCurve = oIntersectCurve
        End If
    End If

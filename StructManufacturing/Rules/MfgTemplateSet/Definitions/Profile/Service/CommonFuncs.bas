...
Private Const EPSILON_PLANAR = 0.005
...
On Error GoTo ErrorHandler
    Dim oTemplateData As TemplData
    Dim nIndex As Long
...
ErrorHandler:
    Err.Raise Err.Number, Err.Source, Err.Description
End Sub
Public Function GetTheTwoEdgesParallelToProfileAxis(oSurfaceBody As IJSurfaceBody, oEdgeElements As IJElements) As IJElements
    Const METHOD = "GetTheTwoEdgesParallelToProfileAxis"
    On Error GoTo ErrorHandler
    ...
    'Algorithm:
    '--usually they are longest
    '--so among the edges, let us return the two longest
    
    Dim i As Long
    Dim dMaxLength1 As Double 'highest
    Dim dMaxLength2 As Double 'second highest
    Dim iLongestEdge1 As Long
    Dim iLongestEdge2 As Long
    For i = 1 To oEdgeElements.count
        Dim oWireBody As IJWireBody
        Set oWireBody = oEdgeElements.Item(i)
        
        Dim oModelBody As IJDModelBody
        Set oModelBody = oWireBody
        Dim dAccuracy As Double
        Dim dAccuracyAchieved As Double
        Dim dLength As Double, dArea As Double, dVolume As Double
        oModelBody.GetDimMetrics dAccuracy, dAccuracyAchieved, dLength, dArea, dVolume
        
        If dLength > dMaxLength2 Then
            If dLength > dMaxLength1 Then
                dMaxLength2 = dMaxLength1
                iLongestEdge2 = iLongestEdge1
                dMaxLength1 = dLength
                iLongestEdge1 = i
            Else
                dMaxLength2 = dLength
    ...
    oOutputEdges.Add oEdgeElements.Item(iLongestEdge1)
    oOutputEdges.Add oEdgeElements.Item(iLongestEdge2)

    Set GetTheTwoEdgesParallelToProfileAxis = oOutputEdges

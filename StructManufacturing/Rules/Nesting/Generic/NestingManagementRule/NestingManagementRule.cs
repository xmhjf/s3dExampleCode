...
//
//      History:
//      June 13, 2013    Kristian Kamph       Creation.
//-----------------------------------------------------------------------------
using System;
...
{
    /// <summary>
    /// The nesting management rule class
    /// </summary>
    public class NestingManagementRule : NestingManagementRuleBase
    {
        /// <summary>
        ...
        public override void ProcessInputXML(string inputXML, out string outputXML)
        {
    ...
    public class NestingService : NestingServiceBase
    {
        ...
        private MetadataManager m_MetaDataManager= null;
        private SiteManager siteMgr= null;
        private Plant activePlant= null;
        private Model modelDB=null;
        private Catalog catalogDB=null;
        private Site site=null;
        private string startTime = DateTime.Now.ToString("yyyy_dd_hh_mm_ss");
        ...
        public string LogDirectory
        {
            get
            {
                if (String.IsNullOrEmpty(logDirectory) || String.IsNullOrWhiteSpace(logDirectory))
                    logDirectory = GetLogDirectoryPath();
        ...
        public void Run(string inputXML, out string outputXML)
        {
            ...
            try
            {
                ...
                CommonLogInfo("Nesting Service called","NestingManagementRuleLog_" + startTime);
                ...
                string sharedContentPath = site.ActivePlant.PlantCatalog.SymbolShare;
            ...
            {
                CommonLogExceptionInfo(ex, "NestingManagementRuleLog_" + startTime);
            }
        ...
        private void ProcessInputXML(XmlDocument inputXmlDocument)
        {
            try
            {
                ...
                for (int i = 0; i < actionNodeList.Count; i++)
                {
                    ...
                    if (actionNode.Attributes.GetNamedItem("NAME").Value == "Alma-RemnantReport") { GetRemnantReport(actionNode, inputXmlDocument); }
        ...
        {
            CommonLogInfo("Getting Nesting System Information", "NestingManagementRuleLog_" + startTime);
            ...
            {
                CommonLogInfo("Missing COMMAND attribute while getting Nesting System Information", "NestingManagementRuleLog_" + startTime);
            }
        }
        ...
        private void GetDBInformation(XmlNode actionNode, XmlDocument inputXmlDocument)
        {
            ...
            try
            {
                CommonLogInfo("Getting DB Information", "NestingManagementRuleLog_" + startTime);
                if(siteMgr == null) siteMgr = MiddleServiceProvider.SiteMgr;
                   ...
                   for (int i = 0; i < siteMgr.Sites.Count; i++)
                    {
                        ...
                        siteNode.SetAttribute("SERVER_NAME", site.Server.ToString());
                        siteNode.SetAttribute("PROVIDER", site.DBProvider);
                        inputXmlDocument.DocumentElement.AppendChild(siteNode);
                            try
                            {
                              XmlElement modelNode = inputXmlDocument.CreateElement("MODEL");
                               modelNode.SetAttribute("NAME", activePlant.Name);
                               modelNode.SetAttribute("SYMBOL_SHARE", activePlant.PlantCatalog.SymbolShare);
        ...
        private void Confirmation(XmlNode actionNode, XmlDocument inputXmlDocument)
        {
            try
            {
                CommonLogInfo("Updating NestData for confirmation", "NestingManagementRuleLog_" + startTime);
                ...
                for (int i = 0; i < oNestDataByFilter.Count(); i++)
                {
                    try
                    {
                        ...
                        CommonLogInfo("Updating NestData for confirmation for NestData: " + bo.ObjectID, "NestingManagementRuleLog_" + startTime);
                        ...
                        if (!(nestData == null))
                        {
                            ...
                            for (int j = 0; j < propertyNodeList.Count; j++)
                            {
                                ...
                                CommonLogInfo("     Updating the attribute: " + propertyName + " the value: " + propertyValue, "NestingManagementRuleLog_" + startTime);
        ...
        {
            CommonLogInfo("Updating NestData for Nesting", "NestingManagementRuleLog_" + startTime);
            try
            {
                ReadOnlyCollection<BusinessObject> oNestDataByFilter;
                oNestDataByFilter = GetParts(actionNode, inputXmlDocument);
                for (int i = 0; i < oNestDataByFilter.Count(); i++)
                {
                    ...
                    {
                        BusinessObject bo = oNestDataByFilter[i];
                        CommonLogInfo("Updating NestData for Nesting for NestData: " + bo.ObjectID, "NestingManagementRuleLog_" + startTime);
                        XmlNodeList propertyNodeList = actionNode.SelectNodes(".//PROPERTY");
                        NestData nestData = bo as NestData;
                        if (!(nestData == null))
                        {
        ...
        {
            CommonLogInfo("Getting Parts", "NestingManagementRuleLog_" + startTime);
            try
            {
        ...
        private void Reset(XmlNode actionNode, XmlDocument inputXmlDocument)
        {
            try
            {
                CommonLogInfo("Updating NestData for Reset", "NestingManagementRuleLog_" + startTime);
                ReadOnlyCollection<BusinessObject> oNestDataByFilter;
                oNestDataByFilter = GetParts(actionNode, inputXmlDocument);
                for (int i = 0; i < oNestDataByFilter.Count; i++)
                {
                    ...
                    if (businessObject.SupportsInterface("IJMfgNestData"))
                    {
                        try
                        {
                            ...
                            CommonLogInfo("Resetting NestData for NestData: " + nestData.ObjectID, "NestingManagementRuleLog_" + startTime);
        ...
        {
            CommonLogInfo("Deleting NestData", "NestingManagementRuleLog_" + startTime);
            try
            {
                ReadOnlyCollection<BusinessObject> oNestDataByFilter;
                oNestDataByFilter = GetParts(actionNode, inputXmlDocument);
                    ...
                    if (businessObject.SupportsInterface("IJMfgNestData"))
                    {
                        try
                        {
                            ...
                            if (outputPath != null && outputPath.Length > 0)
                            {
                                ...
                                try
                                {
                                    CommonLogInfo("Saving Delete XML to: " + outputPath + "\\" + pNestData.ObjectIDForQuery + partName + "DELETE.xml", "NestingManagementRuleLog_" + startTime);
                            ...
                            {
                                CommonLogInfo("No Path Specified for Delete XML for: " + pNestData.ObjectIDForQuery, "NestingManagementRuleLog_" + startTime);
                            }
        ...
        {
            CommonLogInfo("Getting Up-to-date XML", "NestingManagementRuleLog_" + startTime);
            try
            {
                ReadOnlyCollection<BusinessObject> oNestDataByFilter;
                oNestDataByFilter = GetParts(actionNode, inputXmlDocument);
                for (int i = 0; i < oNestDataByFilter.Count; i++)
                {
                    ...
                    if (businessObject.SupportsInterface("IJMfgNestData"))
                    {
                        try
                        {
                            ...
                            if (mfgOutput != null)
                            {
                                CommonLogInfo("Getting Up-to-date XML for: " + mfgOutput.ObjectID, "NestingManagementRuleLog_" + startTime);
                                partName = mfgOutput.Name;
                                ...
                                if (outputPath != null && outputPath.Length > 0)
                                {
                                    CommonLogInfo("Outputting XML to : " + outputPath + "\\" + pNestData.ObjectIDForQuery + partName + ".xml", "NestingManagementRuleLog_" + startTime);
        ...
        {
            CommonLogInfo("Setting Nesting Properties", "NestingManagementRuleLog_" + startTime);
            try
            {
                ReadOnlyCollection<BusinessObject> oNestDataByFilter;
                oNestDataByFilter = GetParts(actionNode, inputXmlDocument);
                ...
                {
                    BusinessObject bo = oNestDataByFilter[i];
                    CommonLogInfo("Setting Nesting properties for: " + bo.ObjectID, "NestingManagementRuleLog_" + startTime);
                    XmlNodeList propertyNodeList = actionNode.SelectNodes(".//PROPERTY");
                    for (int j = 0; j < propertyNodeList.Count; j++)
                    {
                        string propertyName = propertyNodeList.Item(j).Attributes.GetNamedItem("NAME").Value;
                        ...
                        string propertyType = propertyNodeList.Item(j).Attributes.GetNamedItem("TYPE").Value;
                        string propertyValue = propertyNodeList.Item(j).Attributes.GetNamedItem("VALUE").Value;
                        CommonLogInfo("    Updating attribute: " + propertyName + " With value: " + propertyValue, "NestingManagementRuleLog_" + startTime);
                        SetAttributeValue(bo, propertyName, interfaceName, propertyType, propertyValue);
                    }
                }
            }
        ...
        {
            CommonLogInfo("Connecting to model", "NestingManagementRuleLog_" + startTime);
            try
            {
        ...
        {
            CommonLogInfo("Connecting to Site", "NestingManagementRuleLog_" + startTime);
            Site oSite = null;
            try
            {
            ...
            {
                CommonLogExceptionInfo(ex, "NestingManagementRuleLog_" + startTime);
                CommonLogInfo("Exception Occurred While Connecting to Site - " + ex.Message, "NestingManagementRuleLog_" + startTime);
            }
        ...
        {
            CommonLogInfo("Opening Model", "NestingManagementRuleLog_" + startTime);
            try
            {
                Plant oPlant;
        ...
        {
            CommonLogInfo("Setting Permission Group", "NestingManagementRuleLog_" + startTime);
            try
            {
                    ...
                    for (int j = 0; j < oPG.AccessRules.Count; j++)
                    {
                        ...
                        if ((oAR.AccessRight != PGAccessRights.Read) || Thread.CurrentPrincipal.IsInRole(oAR.User))
                        {
                            if (oPG.Name == pgName || (oPG.Parent.ToString() + "\\" + oPG.Name == pgName))
                            {
                                ...
                                CommonLogInfo("Set Active Permission Group " + oPG.Parent.ToString() + "\\" + thisPGName + "." + oAR.User + " - " + oAR.AccessRight.ToString(), "NestingManagementRuleLog_" + startTime);
                                return true;
                            }
                        }
                    }
        ...
        {
            string strCurrentTime = DateTime.Now.ToLongTimeString();
            string strErrLogFile = LogDirectory + strName + ".log";
            using (StreamWriter oErrLogFile = new System.IO.StreamWriter(strErrLogFile, true))
            {
                oErrLogFile.WriteLine(strCurrentTime + " - " + ex.Message);
                oErrLogFile.Flush();
                oErrLogFile.Close();
            }
        }
        ...
        private string CallNestingSystem(string commandName, string DBName, string fileNameOut)
        {
            ...
            {
                CommonLogInfo("Missing client program to call nesting system", "NestingManagementRuleLog_" + startTime);
                return "";
            }
            ...
            if (fileName == installDir + "S3DProductionClient.exe")
            {
                ...
                {
                    CommonLogInfo("Missing configuration file for Remote access to nesting system", "NestingManagementRuleLog_" + startTime);
                    return "";
                }
